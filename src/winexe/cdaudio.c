...
MCIERROR cdAudio_createTrackIndex(void) {
    ...
    err = mciSendCommand(gMciDeviceId, MCI_STATUS, MCI_STATUS_ITEM,
                         (DWORD)&params);
    if (err != 0) {
        return err;
    }
    ...
    ghCdAudioTrackListMemory =
        GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, (gCdAudioTrackCnt + 1) * 4);
    ...
    for (i = 0; i < gCdAudioTrackCnt; ++i) {
        ...
        params.dwTrack = i + 1;
        err = mciSendCommand(gMciDeviceId, MCI_STATUS,
                             MCI_STATUS_ITEM | MCI_TRACK, (DWORD)&params);
        if (err != 0)
            break;
...
MCIERROR cdAudio_open(LPSTR errMsg, ULONG msgSize) {
    ...
    GetPrivateProfileString("Directory", "Type", "FULL", str, sizeof(str),
                            "sonic.ini");
    if (str[0] == 'M') {
        GetPrivateProfileString("Directory", "Play", "D:\\GAME", str,
                                sizeof(str), "sonic.ini");
        drive[0] = str[0];
        drive[1] = ':';
        drive[2] = '\0';
        openParams.lpstrElementName = drive;
        err = mciSendCommand(
            0, MCI_OPEN, MCI_OPEN_SHAREABLE | MCI_OPEN_ELEMENT | MCI_OPEN_TYPE,
            (DWORD)&openParams);
    } else {
    ...
    if (statusParams.dwReturn != MCI_FORMAT_MSF) {
        ...
        err = mciSendCommand(gMciDeviceId, MCI_SET, MCI_WAIT | MCI_TO,
                             (DWORD)&setParams);
        if (err != 0) {
            mciGetErrorString(err, errMsg, msgSize);
            return err;
...
// 00401479
UINT cdAudio_getTrackCnt(void) { return gCdAudioTrackCnt; }
...
MCIERROR cdAudio_play(void) {
    MCIERROR err = mciSendCommand(gMciDeviceId, MCI_PLAY, gMciPlayFlags & -5,
                                  (DWORD)&gMciPlayParams);
    return err;
}
...
MCIERROR cdAudio_switchTrack(long trackId, BOOL bUnknown, HWND hWnd) {
    ...
    if (err == 0) {
        ...
        err = mciSendCommand(gMciDeviceId, MCI_PLAY, gMciPlayFlags,
                             (DWORD)&gMciPlayParams);
        if (err != 0) {
...
MCIERROR cdAudio_seekTrack(int *pTrackId) {
    ...
    GlobalUnlock(ghCdAudioTrackListMemory);
    err = mciSendCommand(gMciDeviceId, MCI_SEEK, MCI_WAIT | MCI_TO,
                         (DWORD)&params);
...
MCIERROR cdAudio_playIfNotStopped(HWND hWnd) {
    if (DAT_00430370 & 2) {
        ...
        return mciSendCommand(gMciDeviceId, MCI_PLAY, gMciPlayFlags,
                              (DWORD)&gMciPlayParams);
    } else {
        return 0;
    }
