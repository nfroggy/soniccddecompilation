...
BOOL switchGameMenuDll(HWND hWnd, char *path, USHORT type) {
    ...
    gpDLLInit =
        (void(__stdcall *)(dllIn *))GetProcAddress(ghGameMenuDll, "DLLInit");
    gpDLLEnd =
        (void (*)(char ***, void **))GetProcAddress(ghGameMenuDll, "DLLEnd");
    gpSWdataSet = (void(__stdcall *)(ushort_union, ushort_union))GetProcAddress(
        ghGameMenuDll, "SWdataSet");
    if (type == 2 || type == 3 || type == 4 || type == 5) {
        gpDLLPaint =
            (void(__stdcall *)(HDC))GetProcAddress(ghGameMenuDll, "DLLPaint");
        gpDLLAVIRealize =
            (void (*)(void))GetProcAddress(ghGameMenuDll, "DLLAVIRealize");
        gpDLLAVISizeChange =
            (void (*)(void))GetProcAddress(ghGameMenuDll, "DLLAVISizeChange");
        gpAVIPause = (void (*)(void))GetProcAddress(ghGameMenuDll, "AVIPause");
        gpAVIResume =
            (void (*)(void))GetProcAddress(ghGameMenuDll, "AVIResume");
    }
    if (type == 3 || type == 4) {
        gpDLLNotify = (int(__stdcall *)(WPARAM, LPARAM))GetProcAddress(
            ghGameMenuDll, "DLLNotify");
    }
    if (type == 6) {
        gpDLLChar =
            (void (*)(WPARAM, LPARAM))GetProcAddress(ghGameMenuDll, "DLLChar");
...
BOOL loadOpening(HWND hWnd, void *hSurf) {
    ...
    if (!gbFullScreenMenuItemEnabled) {
        ...
        enableSubMenuItem(1, ID_OPTIONS_FULLSCREEN,
                          gbFullScreenMenuItemEnabled);
    }
    fillColorwk(0);
    FUN_004051ab();
...
// 0040e313
void unloadOpening(void) { unloadGameMenuDll(); }
...
// 0040e66a
void unloadAviOpen(HWND hWnd) { unloadAvi(hWnd); }

// 0040e681
void unloadAviGood(HWND hWnd) { unloadAvi(hWnd); }

// 0040e698
void unloadAviBad(HWND hWnd) { unloadAvi(hWnd); }

// 0040e6af
void unloadAvipen(HWND hWnd) { unloadAvi(hWnd); }
...
// 0040e757
void realizeMovie(void) { (*gpDLLAVIRealize)(); }
...
// 0040eb1a
void unloadBesttime(void) { unloadGameMenuDll(); }

// 0040eb2a
void unloadVisualmd(void) { unloadGameMenuDll(); }

// 0040eb3a
void unloadSoundtst(void) { unloadGameMenuDll(); }

// 0040eb4a
void unloadStagetst(void) { unloadGameMenuDll(); }
...
// 0040f136
void pauseMovie(void) { (*gpAVIPause)(); }

// 0040f147
void resumeMovie(void) { (*gpAVIResume)(); }
