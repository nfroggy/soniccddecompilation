...
int extractTiles(void ***ppBmpHandles, int *pBmpCnt, char *fileName) {
    ...
    *pBmpCnt = 0;
    ...
    for (i = 0; i < header.cnt; ++i) {
        ...
        err = _hbmpCreate(pDimensions[i][0], pDimensions[i][1], 0,
                          &pBmpHandles[i]);
        if (err != 0)
            break;
        _hbmpGetLockedBuffer(pBmpHandles[i], (void **)&pBmpBuffer, &local_6c);
        expandBitmap(pBmpBuffer, pBmpData, pDimensions[i][0], pDimensions[i][1],
                     paletteOffset, 0, 0);
    ...
    *pBmpCnt = 0;
    ...
    for (i = 0; i < header.cnt; ++i) {
        ...
        for (j = 0; i < 2; ++i) {
            ...
            if (err != 0)
                break;
            _hbmpGetLockedBuffer(pBmpHandles[j + i * 2], (void **)&pBmpBuffer,
                                 &local_6c);
            expandBitmap(pBmpBuffer, pBmpData, pDimensions[i][0],
                         pDimensions[i][1], paletteOffset, 0, 0);
            if (j == 1) {
...
int extractSprites(void ***ppBmpHandles, int *pBmpCnt, char *fileName) {
    ...
    for (i = 0; i < header.cnt; ++i) {
        ...
        expandBitmap(pBmpBuffer, pBmpData, pMeta[i].width, pMeta[i].height,
                     pMeta[i].paletteOffset, 1, hasPadding);
