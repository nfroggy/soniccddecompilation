...
int lzDecompress(cmpHeader *pHeader, char **ppAlloc, char *fileName) {
    ...
    int i;
    char bytes[4];
    if (GetPrivateProfileString("Directory", "Play", "", path, sizeof(path),
                                "sonic.ini")) {
        strcat(path, "\\");
    ...
    if (LZRead(hFile2, (char *)pHeader, sizeof(cmpHeader)) !=
        sizeof(cmpHeader)) {
        LZClose(hFile2);
        return 1;
    }
    *ppAlloc = (char *)GlobalLock(
        GlobalAlloc(GPTR, pHeader->pEnd - sizeof(cmpHeader)));
    if (*ppAlloc == 0) {
        LZClose(hFile2);
        return 2;
    }
...
// 0040c428
void expandBitmap(char *pDest, char *pSrc, short width, short height,
                  UCHAR paletteOffset, int param_6, BOOL hasPadding) {
        ...
        for (x = 0; x < width; ++x) {
            if (!hasPadding || width - 4 > x) {
                if (x & 1) {
                    ...
                    *pCurrentDest =
                        ((*pCurrentSrc & 0xF0) >> 4) + paletteOffset;
                }
