...
/* 004331e8 */ void *gpDLLmeminitFuncs[12] = {
    &SetGrid,       &EAsprset,      0,
    &WaveRequest,   &CDPlay,        &CDPause,
    &ChangeTileBmp, &ReadScoreIndx, &WriteScoreData,
    &SetScoreDate,  &WaveAllStop,   &enableSubMenuItem};
/* 00433218 */ HDC ghDc = 0;
/* 0043321c */ HMODULE ghGameStageDll = 0;
/* 00433220 */ USHORT gUserKey[5] = {VK_LEFT, VK_RIGHT, VK_UP, VK_DOWN,
                                     VK_SPACE};
...
/* 00433338 */ void *gGameMemory[11] = {
    gpMapwk,    gpColorwk,     gpColorwk2,    gpColorwk3,
    gpColorwk4, gpHscrollbuff, &gFadeFlag,    &gKeepWork,
    &ghSurf,    &ghWnd,        &gCrntScorData};
...
BOOL stageLoop(void) {
    ...
    if (gFadeFlag != 0) {
        ...
        if (gStageGameFuncRet != 0 || gbRecordPlayFlag != 0 ||
            gKeepWork.demoflag.w != 0) {
        ...
        if (gStageGameFuncRet != 0) {
                ...
                if (gKeepWork.ta_flag != 0) {
                    ...
                    if (gKeepWork.stageno.b.l == 0) {
                        if (DAT_0042570c == TRUE) {
                            gCrntScorData.clrgood |=
                                1 << gKeepWork.stageno.b.h - 1;
                        }
                    ...
                    if (gKeepWork.stageno.w == 0x0700) {
                        unloadGame();
                        if (gKeepWork.clrspflg_save == 127 ||
                            gCrntScorData.clrgood == 127) {
                            gEndingMovieFlag = 1;
                        } else {
                            gEndingMovieFlag = 2;
                        }
            ...
            if (++DAT_004332b0 >= gFpsFlag1) {
                ...
                mapinit(scrahposiw.w.h, vscroll.w.h, scrbhposiw.w.h,
                        vscroll.w.l);
                makePalette2();
            }
        }
...
void setupTimer(void) {
    gTimeSetEventResult = timeSetEvent(16, 1, timeCallbackFunc,
                                       (DWORD)&gTimerTickCnt, TIME_PERIODIC);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    ...
    MSG msg;
    if (isGameInUse()) {
        MessageBox(ghWnd, "SONIC CD is already in use.", "SONIC CD Error",
                   MB_ICONSTOP);
        return 0;
    }
    if (hPrevInstance == 0) {
    ...
    if (timeBeginPeriod(1) == TIMERR_NOCANDO) {
        MessageBox(ghWnd, "Can't handle the timer resolution",
                   "timeBeginPeriod()", MB_OK);
    }
    setupTimer();
    setupJoystick();
    gControllerId = 2;
...
BOOL makeWindow(HINSTANCE hInstance, int nCmdShow) {
    ...
    hWnd = CreateWindowEx(0, "SONICCDClass", "SONIC CD",
                          WS_MINIMIZEBOX | WS_SYSMENU | WS_CAPTION, rect.left,
                          rect.top, rect.right - rect.left + 1,
                          rect.bottom - rect.top + 1, 0, 0, hInstance, 0);
    if (hWnd == 0) {
        return FALSE;
    }
    ghWnd = hWnd;
...
// 00408704
BOOL isCpuPentium(void) { return TRUE; }
...
BOOL isComputerNec(void) {
    ...
    GetPrivateProfileString("boot.description", "system.drv", "", buffer,
                            sizeof(buffer), "system.drv");
...
BOOL isCdromPresent(void) {
    ...
    if (result.w.l == (USHORT)HFILE_ERROR) {
        MessageBox(0, "CD check file is not found.", "Sonic Error",
                   MB_ICONSTOP);
        return FALSE;
    }
    _lclose(result.l);
...
// 0040889f
void __stdcall enableSubMenuItem(int subMenuPos, UINT menuItemId,
                                 BOOL bEnable) {
    HMENU hMenu;
    UINT flags;
    hMenu = GetSubMenu(GetMenu(ghWnd), subMenuPos);
...
void modifyControllerMenuItemText(USHORT controllerId) {
    ...
    if (controllerId == 1) {
        ...
        ModifyMenu(hMenu, ID_OPTIONS_USEKEYBOARD, MF_BYCOMMAND | MF_STRING,
                   ID_OPTIONS_USEKEYBOARD, buffer);
    } else {
...
void __stdcall toggleSoundQuality(void) {
    ...
    if (gbBetterSoundQuality) {
        ...
        ModifyMenu(hMenu, ID_BETTERSOUNDQUALITY, MF_BYCOMMAND | MF_STRING,
                   ID_BETTERSOUNDQUALITY, str);
    } else {
...
void retrieveHelpFilePath(void) {
    if (GetPrivateProfileString("Directory", "Install", "", gHelpFilePath,
                                sizeof(gHelpFilePath), "sonic.ini") != 0) {
...
void toggleMenuBar(void) {
    ...
    if (!gbFullScreen) {
        ...
        MoveWindow(ghWnd, rect1.left, rect1.top, rect2.right - rect2.left,
                   rect2.bottom - rect2.top, TRUE);
    }
...
void changeControls(void) {
    ...
    showSonicDlg(ghWnd, "SONICDLG_KEYS", "KeySettingDialog",
                 (LPARAM)gUserKeyTemp);
...
LRESULT __stdcall WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        ghDc = GetDC(ghWnd);
        hCursor = SetCursor(LoadCursor(0, IDC_WAIT));
        GetPrivateProfileString("Directory", "Type", "MINIMAL", buffer,
                                sizeof(buffer), "SONIC.INI");
        if (buffer[0] == 'F') {
            GetPrivateProfileString("Secret", "Super", "OFF", buffer,
                                    sizeof(buffer), "SONIC.INI");
        ...
        if (buffer[0] != 'J') {
            ...
            // xor instead of and
            versionInfo = HIBYTE(LOWORD(versionInfo)) +
                          (LOBYTE(LOWORD(versionInfo)) << 8);
            if (versionInfo < 0x35F) {
                MessageBox(0,
                           "You must have a Windows® 95\n inside your system "
                           "to run Sonic the Hedgehog CD.",
                           "Sonic Error", MB_ICONSTOP);
                return -1;
            }
            if (isCpuPentium() == FALSE) {
                MessageBox(
                    0,
                    "You must have a Pentium® Processer (or better)\ninside "
                    "your system to run Sonic the Hedgehog CD.",
                    "Sonic Error", MB_ICONSTOP);
                return -1;
            }
            SetFocus(ghWnd);
        ...
        GetPrivateProfileString("SOUND", "QUALITY", "GOOD  ", buffer,
                                sizeof(buffer), "SONIC.INI");
        if (buffer[0] == 'B') {
        ...
        if ((int)ghSonicDlg <= 32) {
            MessageBox(0, "Can't load SONICDLG.DLL", "Sonic Error",
                       MB_ICONSTOP);
            return -1;
        }
        pInitKeySettings = (void(__stdcall *)(HWND, USHORT *))GetProcAddress(
            ghSonicDlg, "InitKeySettings");
        if (pInitKeySettings == 0) {
            MessageBox(0,
                       "Can't GetProcAddress() for "
                       "SONICDLG.DLL:\t\t\t\t\tInitKeySettings()",
                       "Sonic Error", MB_ICONSTOP);
            return -1;
        }
    ...
    case WM_DESTROY:
        ...
        if (gbBetterSoundQuality) {
            WritePrivateProfileString("SOUND", "QUALITY", "BETTER",
                                      "SONIC.INI");
        } else {
            WritePrivateProfileString("SOUND", "QUALITY", "GOOD  ",
                                      "SONIC.INI");
        }
        WaveAllStop();
        CDPause();
    ...
    case WM_COMMAND:
        ...
        if (wParam >= ID_STAGE_SPECIAL_R1 &&
            wParam <= ID_STAGE_SPECIAL_R8) { /* Special Stage */
            gKeepWork.play_start = 0;
            hCursor = SetCursor(LoadCursor(0, IDC_WAIT));
        ...
        case ID_FUNC_FLAGS:
            wsprintf(buffer, "generate_flag=%02x \n\rtime_flag=%02x",
                     gKeepWork.generate_flag, gKeepWork.time_flag);
...
BOOL loadGameDll(LPCSTR path) {
    ...
    gpDLLmeminit = (void(__stdcall *)(char ***, void **))GetProcAddress(
        ghGameStageDll, "DLL_meminit");
    gpDLLmemfree =
        (void (*)(void))GetProcAddress(ghGameStageDll, "DLL_memfree");
    gpSWdataSet = (void(__stdcall *)(ushort_union, ushort_union))GetProcAddress(
        ghGameStageDll, "SWdataSet");
    gpGetvscroll = (int (*)(void))GetProcAddress(ghGameStageDll, "Get_vscroll");
    gpGetscrahposiw =
        (int (*)(void))GetProcAddress(ghGameStageDll, "Get_scra_h_posiw");
    gpGetscrbhposiw =
        (int (*)(void))GetProcAddress(ghGameStageDll, "Get_scrb_h_posiw");
    gpFadeProc = (int (*)(void))GetProcAddress(ghGameStageDll, "FadeProc");
    gpSetDebugFlag = (void(__stdcall *)(unsigned int))GetProcAddress(
        ghGameStageDll, "SetDebugFlag");
    gpGetRoundStr = (void(__stdcall *)(unsigned short, unsigned char,
                                       char *))GetProcAddress(ghGameStageDll,
                                                              "GetRoundStr");
    gpSpecialblockchg = (void(__stdcall *)(
        unsigned short *, unsigned short *, unsigned short *,
        unsigned short *))GetProcAddress(ghGameStageDll, "Special_block_chg");
...
BOOL loadSpecialStage(int stageMenuId) {
    ...
    if (stageMenuId != 0) {
        ...
        for (i = 0; i < 7; ++i) {
            if (!(gKeepWork.clrspflg_save & 1 << gKeepWork.stagenm))
                break;
            ++gKeepWork.stagenm;
            if (gKeepWork.stagenm > 6) {
                gKeepWork.stagenm = 0;
...
BOOL setupJoystick(void) {
    ...
    if (joyGetDevCaps(0, &gJoyCapsInfo, sizeof(gJoyCapsInfo)) ==
        MMSYSERR_NODRIVER) {
        return FALSE;
    }
...
void updateSwdata(UCHAR buttonsPressed, ushort_union *pSwdata) {
    ...
    if (DAT_00433258 != 0) {
        if (gRecordingBufferReadWriteCnt == 0x1800) {
            ...
            gRecordingBuffer[gRecordingBufferReadWriteCnt * 2 + 1] =
                pSwdata->b.l;
    ...
    if (gbRecordPlayFlag) {
        // xor instead of and
        if (((USHORT *)gRecordingBuffer)[gRecordingBufferReadWriteCnt] ==
            0xFFFF) {
            gbRecordPlayFlag = FALSE;
            gRecordingBufferReadWriteCnt = 0;
        } else {
...
void changeMusic(void) {
    ...
    if (gbCdAudioOpenSuccess == TRUE) {
        if (gCurrentCdAudioTrack == 1 || gCurrentCdAudioTrack == 27 ||
            gCurrentCdAudioTrack == 30 || gCurrentCdAudioTrack == 29 ||
            gCurrentCdAudioTrack == 28) {
            unknown = FALSE;
        } else {
            unknown = TRUE;
        }
...
void readRecording(void) {
    ...
    if (hFile == -1) {
        ...
        if (gRecordingBufferReadWriteCnt == -1) {
            ...
            ((USHORT *)gRecordingBuffer)[gRecordingBufferLogicalEntryCnt * 2] =
                0xFFFF;
            gRecordingBufferReadWriteCnt = 0;
            _lclose(hFile);
        }
...
void writeRecording(void) {
    ...
    if (hFile == -1) {
        ...
        gRecordingBufferReadWriteCnt =
            _lwrite(hFile, (LPCSTR)gRecordingBuffer,
                    gRecordingBufferLogicalEntryCnt * 2);
        _lclose(hFile);
    }
}

// 0040c00c
short FUN_0040c00c(HWND hWnd) { return 0; }
...
int showSonicDlg(HWND hWnd, LPCSTR resourceId, LPCSTR dialogId,
    ...
    if (dlgProc == 0) {
        wsprintf(errMsg, "Can't GetProcAddress() for SONICDLG.DLL:%s()",
                 dialogId);
...
void loadIni(void) {
    char buffer[256];
    if (GetPrivateProfileString("Secret", "User", "", buffer, sizeof(buffer),
                                "SONIC.INI") == 0)
        return;
...
BOOL isDisplay256Colors(void) {
    ...
    MessageBox(0,
               "Your computer display setting must be 256 color to play Sonic "
               "the Hedgehog CD.",
               "Sonic Error", MB_ICONSTOP);
