...
int openWaveOut(HWND hWnd) {
    ...
    for (i = 0; i < 6; ++i, ++pWaveInfo) {
        ...
        waveFormat.cbSize =
            waveOutOpen(&pWaveInfo->hWaveout, WAVE_MAPPER, &waveFormat,
...
int loadPcmFile(char *path) {
    ...
    if (ret == 0) {
        gpPcmChunkIndex =
            (UINT *)allocAndLockMemory(GMEM_ZEROINIT, header.chunkCnt * 4);
...
// 0040d967
int getPcmDataChunkCount(void) { return gPcmDataChunkCnt; }
...
int playWaveRequest(HWND hWnd, waveInfo *pWaveInfo, int ReqNo) {
    ...
    if (waveOutPrepareHeader(pWaveInfo->hWaveout, &pWaveInfo->waveHdr,
                             sizeof(pWaveInfo->waveHdr)) != MMSYSERR_NOERROR) {
        return 2;
    }
    if (waveOutWrite(pWaveInfo->hWaveout, &pWaveInfo->waveHdr,
                     sizeof(pWaveInfo->waveHdr)) != MMSYSERR_NOERROR) {
        waveOutUnprepareHeader(pWaveInfo->hWaveout, &pWaveInfo->waveHdr,
                               sizeof(pWaveInfo->waveHdr));
        return 3;
    }
...
int wavePause(int waveIndex) {
    ...
    if (gWaveInfos[waveIndex].busy) {
        if (waveOutPause(gWaveInfos[waveIndex].hWaveout) ==
...
int waveRestart(int waveIndex) {
    ...
    if (gWaveInfos[waveIndex].busy) {
        if (waveOutRestart(gWaveInfos[waveIndex].hWaveout) ==
            MMSYSERR_INVALHANDLE) {
            return 1;
        }
    }
