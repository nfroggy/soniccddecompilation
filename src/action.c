...
void actionsub(sprite_status* pActwk) {
  ...
  if (lpKeepWork->GamePass != 0) return;
  ...
  if (flag) {
    ...
    if ((Uint16)scra_v_posit.w.h < 256) {
      ...
      if ((Uint16)ypos >= 2048) ypos -= 2048;
    }
    else if ((Uint16)scra_v_posit.w.h >= 1792) {

      if ((Uint16)ypos < 256) ypos += 2048;
    }
      ...
      if (pActwk->actno && pActwk->patbase != 0) {
        ...
        if (flag) {
          ...
          if ((Uint16)pScrVposi->w.h < 256) {
            ...
            if ((Uint16)yposi >= 2048) yposi -= 2048;
          }
          else if ((Uint16)pScrVposi->w.h >= 1792) {

            if ((Uint16)yposi < 256) yposi += 2048;
          }
          yposi -= pScrVposi->w.h;
          yposi += 128;
        }
...
void spatset(Sint16 xposi, Sint16 yposi, sprite_status* pActwk, sprite_data* sprdat, Sint16 cnt) {
  Uint8 flag;
  Sint16 x, y;
  Uint16 reverse;
    ...
    if (sprdat->index) {
      ...
      switch (flag) {
        case 0:
          ...
          break;
        case 1:
          x = xposi + (Sint16)(-sprdat->xoff - (Sint16)SprBmp[sprdat->index].xs);
          y = yposi + (Sint16)sprdat->yoff;
          break;
        case 2:
          x = xposi + (Sint16)sprdat->xoff;
          y = yposi + (Sint16)(-sprdat->yoff - (Sint16)SprBmp[sprdat->index].ys);
          break;
        case 3:
          x = xposi + (Sint16)(-sprdat->xoff - (Sint16)SprBmp[sprdat->index].xs);
          y = yposi + (Sint16)(-sprdat->yoff - (Sint16)SprBmp[sprdat->index].ys);
