...
extern sprite_pattern* markerpat[]; extern Uint8* markerchg[]; extern sprite_pattern* bariapat[]; extern Uint8* bariachg[]; extern sprite_pattern* bakupat[]; extern Uint8* bakuchg[]; extern sprite_pattern* mizukipat[]; extern Uint8* mizukichg[]; extern sprite_pattern* futapat[]; extern Uint8* futachg[]; extern sprite_pattern* exit2pat[]; extern Uint8* exit2chg[];
static Uint8 flowchg0[4] = { 3, 0, 1, 255 }, flowchg1[6] = { 3, 2, 3, 2, 3, 252 }, flowchg2[6] = { 1, 5, 5, 4, 6, 252 }, flowchg3[4] = { 19, 6, 7, 255 };
...
char col_chk(sprite_status* thingwk, sprite_status* sonicwk) {
  ...
  cal_posi = (Sint16)thingwk->sprhsize + (sonicwk->xposi.w.h - thingwk->xposi.w.h);
  if (cal_posi < 0) return 0;
  if (cal_posi >= (thingwk->sprhsize * 2)) return 0;
  cal_posi = (Sint16)thingwk->sprvsize + (sonicwk->yposi.w.h - thingwk->yposi.w.h);
  if (cal_posi < 0) return 0;
...
void muteki_sub(sprite_status* bariawk) {
  ...
  cal_no = cal_no * 24 + 4;
  ((ushort_union*)&ppw_offset)->b.l = ((ushort_union*)&ppw_offset)->b.l - cal_no;
...
void ball_move(sprite_status* ballwk) {
  ...
  if (ballwk->userflag.b.h == 0) {
    ...
    goto label1;
  } else if ((Uint8)ballwk->userflag.b.h >= 2) { ball_rd7(ballwk); return; }
