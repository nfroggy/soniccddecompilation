...
static void togeita_dataset(sprite_status *trapwk, sprite_status *new_actwk,
    ...
    new_actwk->xposi.w.h =
        ((Sint16 *)trapwk)[29] + (Sint16)(char)togeita_data[patchg_point + 1];
...
static Uint8 togeita_plchk_1(sprite_status *trapwk, Sint16 cal_x) {
    ...
    if ((position_check = actwk[0].xposi.w.h - ((Sint16 *)trapwk)[29] + cal_x) <
        0)
        return 0;
...
static void itaset_chk(sprite_status *trapwk) {
    ...
    if (trapwk->actfree[17] < 5) {
        ...
        new_actwk->yposi.w.h =
            ((Sint16 *)trapwk)[27] - (Sint16)((char)trapwk->actfree[17] * 16);
