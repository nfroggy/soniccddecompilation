...
void (*act_tbl[67])(sprite_status *) = {
    ...
    &shooter, &noact,    &chibi_coli, &spring,   &exit2,    &exit2_set,
    &futa,    &mizukiri, &koma8,      &ring,     &flyring,  &noact,
    &marker,  &bigring,  &gene,       &mosugu,   &goal,     &bakuha,
    &item,    &item2,    &iwa,        &score,    &beam,     &noact,
    &flower,  &pipe8,    &sw,         &haguruma, &bigbom,   &kabuto,
    &kuzure8, &legmeca,  &noact,      &noact,    &tobira,   &noact,
    &jettoge, &dai8,     &dango,      &noact,    &noact,    &gater8,
    &noact,   &friend,   &togeita,    &piston,   &okusieso, &togedair8,
    &noact,   &noact,    &noact,      &clear,    &over,     &title,
    &shut,    &kaiten,   &noact,      &noact,    &noact,    &noact,
    &et};
