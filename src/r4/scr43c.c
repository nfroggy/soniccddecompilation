...
Uint16 scr_dir_tbl[6] = {4, 0, 3479, 0, 2048, 96};
...
void playposiset(void) {
    ...
    Uint16 playpositbl[4] = {80, 364, 976, 940};
    ...
    if (lpKeepWork->demoflag.w) {
        xWk = actwk[0].xposi.w.h = playpositbl[2];
        yWk = actwk[0].yposi.w.h = playpositbl[3];
    } else {
        if (plflag != 0) {
            ...
            xWk = actwk[0].xposi.w.h = playpositbl[0];
            yWk = actwk[0].yposi.w.h = playpositbl[1];
        }
    }
    if ((Uint16)xWk > 160) {
        xWk -= 160;
...
void scrollwrtadva(void) {
    ...
    scrollwrtb(pScrFlag, pMapWk, VramBase, wH_posiw, wV_posiw);
