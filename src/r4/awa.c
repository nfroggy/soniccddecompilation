...
pActwk->xposi.w.h = (Sint16)(char)awasintbl[pActwk->direc.b.h++ & 127] + ((Sint16*)pActwk)[26];
...
void awamaster(sprite_status* pActwk) {
  ...
  if (!((Uint16*)pActwk)[29]) {
    ...
    if (--((Sint16*)pActwk)[30] >= 0) goto label3;
...
label1:
  ...
  if (pActwk->actflg & 128) {
    if (actwkchk(&pNewactwk) == 0) {
      pNewactwk->actno = 32;
      pNewactwk->xposi.w.h = pActwk->xposi.w.h + (Sint16)((random() & 15) - 8);
