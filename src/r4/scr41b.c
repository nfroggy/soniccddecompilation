...
void playposiset(void) {
    ...
    if (plflag != 0) {
        ...
        if (demoflag.w < 0) {
            ...
            i = (enddemono - 1) * 4;
            xWk = actwk[0].xposi.w.h = endplpositbl[i++];
            yWk = actwk[0].yposi.w.h = endplpositbl[i++];
        } else {
...
void scrbinit(Sint16 yWk) {
    ...
    data = scra_v_posit.w.h;
    data >>= 1;

    scrb_v_posit.w.h = data;
    scrb_v_posit.w.l = 0;
    scrc_v_posit.w.h = data;
    scrz_v_posit.w.h = data;
