...
Sint32 game(void) {
  if (swdata1.b.l & 128) {
    if (pauseflag.b.h) {
      if (swdata2.w & 32896) {
        if (PauseIcon != 0) pauseflag.b.h = 0, PauseIcon = 0;
...
void water1chk(void) {
  ...
  if ((time_flag & 127) == 1) {
    wD1 = 640;
    if ((Uint16)actwk[0].xposi.w.h >= 1504) wD1 = 608;
  }
  else {
...
for (i = 0; watercolitbl[i].w >= 0; i += 7) {
    if (actwk[0].xposi.w.h < watercolitbl[i].w) continue;
    if (actwk[0].xposi.w.h >= watercolitbl[i + 2].w) continue;
    if (actwk[0].yposi.w.h < watercolitbl[i + 1].w) continue;
    if (actwk[0].yposi.w.h >= watercolitbl[i + 3].w) continue;

    if (((char*)&watercolitbl[i + 6])[1] == 1) {
      if (!(switchflag[((char*)&watercolitbl[i + 6])[0]] & 64)) return;
    }
