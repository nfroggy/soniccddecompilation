...
static Sint16 fcol3a[17] = {15,  316, 326, 411, 430, 131, 132, 137, 138,
                            119, 118, 128, 127, 126, 125, 124, 130};
static Sint16 fcol3b[3] = {1, 325, 326};
static Sint16 fcol3c[17] = {15,  316, 326, 0,   0,   131, 132, 137, 138,
                            119, 118, 128, 127, 126, 125, 124, 130};
static Sint16 fcol3d[17] = {15,  316, 326, 357, 358, 131, 132, 137, 138,
                            119, 118, 128, 127, 126, 125, 124, 130};
...
void boyo(void) {
    ...
    case 0:
        if (!bycol_sub0(actwk[0].xposi.w.h - (Sint16)actwk[0].sprhs,
                        actwk[0].yposi.w.h + (Sint16)actwk[0].sprvsize)) {

            bycol_sub0(actwk[0].xposi.w.h + (Sint16)actwk[0].sprhs,
                       actwk[0].yposi.w.h + (Sint16)actwk[0].sprvsize);
        }
        break;
    ...
    case 192:
        if (!bycol_sub0(actwk[0].xposi.w.h + (Sint16)actwk[0].sprhs,
                        actwk[0].yposi.w.h - (Sint16)(actwk[0].sprvsize - 6))) {
...
void bob(Sint16 i, Sint16 cal_x, Sint16 cal_y, Sint16 block_back) {
    ...
    sinset((char)atan_sonic(cal_x - actwk[0].xposi.w.h,
                            cal_y - actwk[0].yposi.w.h),
           &tmp_sin, &tmp_cos);
...
void bob2(Sint16 i, Sint16 cal_x, Sint16 cal_y, Sint16 block_back) {
    if (((Uint16)cal_x & 15) < 8) {
        if (block_back & 2048)
            bob0(i, cal_x, cal_y, block_back);
        else
            bob1(i, cal_x, cal_y, block_back);
    } else {
        if (block_back & 2048)
            bob1(i, cal_x, cal_y, block_back);
        else
...
void bob3(Sint16 i, Sint16 cal_x, Sint16 cal_y, Sint16 block_back) {
    static Uint8 bydcol0[16] = {1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6};
    static Uint8 bydcol1[16] = {6, 6, 7, 7,  7,  8,  8,  8,
                                9, 9, 9, 10, 10, 10, 11, 11};
    static Uint8 bydcol2[16] = {11, 12, 12, 12, 13, 13, 13, 14,
...
void bob5(Sint16 i, Sint16 cal_x, Sint16 cal_y, Sint16 block_back) {
    if (((Uint16)cal_x & 15) < 8) {
        if (block_back & 2048)
            ...
            bob_s0(i, cal_x, cal_y, block_back);
    } else {
        if (block_back & 2048)
            bob_s0(i, cal_x, cal_y, block_back);
        else
