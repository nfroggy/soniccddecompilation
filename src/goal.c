...
void gene(sprite_status* pActwk) {
  ...
  if (pActwk->r_no0 != 10) {
    ...
    if ((Uint32)pActwk->r_no0 >= 6) return;
  }
...
iD0 = pPlaywk->yposi.w.h - pActwk->yposi.w.h;
  ...
  if ((Uint16)iD0 >= 256) return;

  if ((Uint16)pPlaywk->xposi.w.h < (Uint16)pActwk->xposi.w.h) return;
...
void goal_move0(sprite_status* pActwk) {
  ...
  if (iD0 < 0) return;
  if ((Uint16)iD0 >= 256) return;
  if ((Uint16)pActwk->xposi.w.h >= (Uint16)pPlaywk->xposi.w.h) return;
  scralim_left = scra_h_posit.w.h;
  scralim_n_left = scra_h_posit.w.h;
  pltime_f = 0;
  pActwk->actfree[0] = 120;
