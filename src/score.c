...
static void scorewrt2(sprite_data* pSprdat, Uint32 lDispVal, Uint32* subval, Sint16 sD6, Sint16 zero_disp);
...
static void timewrt0(sprite_data* pSprdat, Uint32 lDispVal, Uint32* subval, Sint16 sD6);
...
void scoreup(Uint32 updata) {
  ...
  if (extrascore > (Uint32)plscore) return;
...
if (pl_suu_f) {
  ...
  if (bonus_f) {
    bonus_f = 0;
    if (stageno.w == 1282) pSprpat = &bonuspat0;
    else pSprpat = &bonuspat;
