...
Sint16 hit_v(sprite_status *pActwk, sprite_status *pPlayerwk, Sint16 iHitXs,
             Sint16 iHitHs) {
    Sint16 iHitYs;
    Sint16 iHitVs;
    Sint16 iD4, iD5;
...
Sint16 hit_x(sprite_status *pActwk, sprite_status *pPlayerwk, Sint16 iHitXs,
             Sint16 iD5) {
    if (pActwk->r_no1 == 1)
        return hit_e(pActwk, pPlayerwk);
    if (pActwk->actno == 10)
...
void hit_set(sprite_status *pActwk, sprite_status *pPlayerwk) {
    ...
    if (iHitActno != 0) {
        ...
        if (pActwk->xspeed.w == 0 && pHitAct->xspeed.w == 0)
            return;

        if (pPlayerwk->xposi.w.h >= pActwk->xposi.w.h &&
            pPlayerwk->xposi.w.h >= pHitAct->xposi.w.h)
            return;
...
#if defined(R41A)
        if (pActwk->actno == 40 && pHitAct->actno == 39)
            return;
#endif

        if (stageno.w == 768 && pActwk->actno == 39 && pHitAct->actno == 39)
            return;
