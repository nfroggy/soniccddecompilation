...
if (hSprMenu[y][colom] != 0)
  ...
  if (code == 32) indx = 89;
  else if (code == 44) indx = 90;
  else if (code == 46) indx = 91;
  else if (code == 45) indx = 72;
  else if (code < 65) indx = code - 48;
  else indx = code - 55;
...
s_ctx->spr_level[hSprMenu[y][colom]] = 60 - infoSprtBmp[indx].order;
  ...
  hmx_sprite_set_position_module(s_ctx->sprites[hSprMenu[y][colom]], point.x, point.y);

  hmx_sprite_set_bitmap_module(s_ctx->sprites[hSprMenu[y][colom]], s_ctx->spr_bitmaps[indx]);
  hmx_sprite_set_flags_module(s_ctx->sprites[hSprMenu[y][colom]], 0);
