...
static Uint32 CreateCharSprt(Sint32 nMenuNum, Sint32 colom, char code, Sint32 color);
...
if (hSprMenu[nMenuNum][colom] != 0)
  ...
  {
    if (code == 46) indx = 82;
    else if (code < 65) indx = code - 3;
    else indx = code - 10;
  }
  else
  {
    if (code == 46) indx = 81;
    else if (code < 65) indx = code - 39;
    else indx = code - 46;
  }
...
hmx_sprite_set_bitmap_module(s_ctx->sprites[hSprMenu[nMenuNum][colom]], s_ctx->spr_bitmaps[indx]);
  hmx_sprite_set_flags_module(s_ctx->sprites[hSprMenu[nMenuNum][colom]], 0);
  return 1;
}
...
s_ctx->spr_level[hMesMenu] = infoSprtBmp[indx].order;
  ...
  hmx_sprite_set_bitmap_module(s_ctx->sprites[hMesMenu], s_ctx->spr_bitmaps[indx]);
