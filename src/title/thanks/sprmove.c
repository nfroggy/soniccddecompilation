...
static Sint16 s_metalchk(sprite_status_thanks *pActwk);
...
static void emmy_exeset(sprite_status_thanks *pActwk,
...
static Sint16 killchk(sprite_status_thanks *pActwk);
...
static Sint16 chk_sonicposi(sprite_status_thanks *pActwk,
                            sprite_status_thanks *pA6, Sint16 *d5, Sint16 *d6);
static Sint16 myposichk(sprite_status_thanks *pActwk);
static Sint16 centerchk(sprite_status_thanks *pActwk);
static Sint16 dsplaychk(sprite_status_thanks *pActwk);
...
void sonic_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[14])(sprite_status_thanks *) = {
        &sonicinit,  &s_runset,   &sonicrun,  &s_turnset, &sonicturn,
        &s_standset, &sonicstand, &s_waitset, &sonicwait, &s_ballset,
        &sonicball,  &sonicexit,  &sonicpit,  &sonicesc};
...
static void sonicrun(sprite_status_thanks *pActwk) {
    ...
    ld0.l = random();
    ld0.l &= 32767;
    ld0.w.l = (Sint32)ld0.w.l % 1536;

    ld0.l = random();
    ld0.l &= 32767;
    ld0.w.l = (Sint32)ld0.w.l % 512;
    if (ld0.w.l == 0) {
        pActwk->EXE_NO = 9;
        return;
    }
...
static void sonicwait(sprite_status_thanks *pActwk) {
    ...
    pMetal = &actwk[2];
    pMetal->XPOSI.w.h &= 511;
    if (chk_sonicposi(pActwk, pMetal, &d5, &d6))
        pActwk->SPR_FLG |= 128;
    else
        pActwk->SPR_FLG &= 127;
    pMetal->SPR_FLG &= 251;
...
static void sonicpit(sprite_status_thanks *pActwk) { pit_in(pActwk); }
...
void emie_ctrl(sprite_status_thanks *pActwk) {
    ...
    void (*acttbl[16])(sprite_status_thanks *) = {
        &emmyinit, &e_runinit,  &emmymove,   &e_kissset, &emmykiss, &e_jumpset,
        &emmyjump, &e_catchset, &emmy_catch, &e_turnset, &emmyturn, &e_standset,
        &emmymoji, &emmyexit,   &emmypose,   &emmyesc};
...
static void e_runinit(sprite_status_thanks *pActwk) {
    ...
    if (chk_sonicposi(pActwk, &actwk[0], &d5, &d6)) {
        pActwk->X_SPEED.l *= -1;
        pActwk->SPR_FLG |= 128;
    } else {
        pActwk->SPR_FLG &= 127;
...
static void e_turnset(sprite_status_thanks *pActwk) {
    ...
    pActwk->SPR_FLG |= 128;
    if (chk_sonicposi(pActwk, &actwk[0], &d5, &d6) == 0) {
        pActwk->SPR_FLG &= 127;
        pActwk->X_OFFSET *= -1;
        pActwk->X_SPEED.l *= -1;
    }
...
static void emmyesc(sprite_status_thanks *pActwk) { escape_chk(pActwk); }

void metals_ctrl(sprite_status_thanks *pActwk) {
    sprite_status_thanks *pSonic;
    void (*acttbl[16])(sprite_status_thanks *) = {
        &metalsinit,  &m_flyset,   &metalsfly,       &m_brakeset,
        &metalsbrake, &m_stopset,  &metalsstop,      &m_standset,
        &metalstand,  &m_startset, &metalstartcttbl, &m_eraset,
        &metalsera,   &metalsexit, &metalspit,       &metalsesc};
...
static void metalsfly(sprite_status_thanks *pActwk) {
    ...
    if (pActwk->TM_CNT < 0) {
        ...
        chk_sonicposi(pActwk, &actwk[0], &d5, &d6);
        if (d5 < 40)
            return;

        chk_sonicposi(pActwk, &actwk[1], &d5, &d6);
        if (d5 < 40)
            return;
...
static void metalspit(sprite_status_thanks *pActwk) { pit_in(pActwk); }

static void metalsesc(sprite_status_thanks *pActwk) { escape_chk(pActwk); }

void heart_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&heartinit, &heartmove};
...
void pocky_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&pockyinit, &animalmove};
...
void pecky_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&peckyinit, &animalmove};
...
void ricky_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&rickyinit, &animalmove};
...
void sheep_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&sheepinit, &animalmove};
...
void flick_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&flickyinit, &birdmove};
...
void inco_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&incoinit, &birdmove};
...
void hato_ctrl(sprite_status_thanks *pActwk) {
    void (*acttbl[2])(sprite_status_thanks *) = {&hatoinit, &birdmove};
...
static void sinhmove(sprite_status_thanks *pActwk) {
    ...
    ld0.l = random();
    ld0.l &= 32767;
    ld0.w.l = (Sint32)ld0.w.l % pActwk->X_WIDE;
...
static void sinvmove(sprite_status_thanks *pActwk) {
    ...
    ld0.l = random();
    ld0.l &= 32767;
    ld0.w.l = (Sint32)ld0.w.l % pActwk->Y_WIDE;
    pActwk->SIN_CNT += ld0.w.l;
    if (pActwk->SIN_CNT >= 511) {
        pActwk->SIN_CNT -= 511;
    }
...
static Sint16 myposichk(sprite_status_thanks *pActwk) {
    ...
    if (pActwk->X_SPEED.w.h >= 0) {
        if (pActwk->XPOSI.w.h < 264)
            return 0;
        if (pActwk->XPOSI.w.h < 304)
            return -1;
        if (pActwk->XPOSI.w.h < 448)
            return 0;
        if (pActwk->XPOSI.w.h < 496)
            return -1;
        return 0;
    } else {
        if (pActwk->XPOSI.w.h < 40)
            return -1;
        if (pActwk->XPOSI.w.h < 304)
            return 0;
...
static Sint16 centerchk(sprite_status_thanks *pActwk) {
    ...
    if (!(pActwk->SPR_FLG & 128)) {
        if (pActwk->XPOSI.w.h < 112)
            return 0;
        if (pActwk->XPOSI.w.h > 120)
            return 0;
        return -1;
    } else {
        if (pActwk->XPOSI.w.h < 168)
            return 0;
        if (pActwk->XPOSI.w.h > 176)
            return 0;
        return -1;
    }
}
