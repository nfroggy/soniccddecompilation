...
extern void(*hmx_ddagrid_set_scan_module)(hmx_ddagrid*, Sint32, Sint32, Sint32, Sint32, Sint32);
...
static void OEUpdatePlanet(void) {
  ...
  if (*infoSprtBmp[14].lphSpr != 0) {
    ...
    hmx_sprite_set_position_module(s_ctx->sprites[14], ptPlnt[num].point.x, ptPlnt[num].point.y);
...
if (nTimerCunt == 0) {
  ...
  for (i = 0; i < 92; ++i) {
    ...
    hmx_ddagrid_set_scan_module((hmx_ddagrid*)s_ctx->grids[2], i, RotateMorphValues[i].StartX, RotateMorphValues[i].DeltaX, RotateMorphValues[i].StartY, RotateMorphValues[i].DeltaY);
  }
  if (++incY >= 128) incY = 0;
}
...
if (swData1 & 24320) {
        ...
        switch (testCheckMode) {
          ...
          case 1:   if (swData1 & 512) ++testCheckMode;
                    else if (swData1 & 256) testCheckMode = 12;
                    else if (swData1 & 2048) testCheckMode = 20;
                    else testCheckMode = 0;
                    break;
          case 2:   if (swData1 & 512) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 3:   if (swData1 & 512) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 4:   if (swData1 & 1024) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 5:   if (swData1 & 2048) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 6:   if (swData1 & 16384) {
                    ...
                    break;
          case 12:  if (swData1 & 512) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 13:  if (swData1 & 512) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 14:  if (swData1 & 1024) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 15:  if (swData1 & 2048) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 16:  if (swData1 & 16384) {
...
case 20:  if (swData1 & 2048) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 21:  if (swData1 & 256) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 22:  if (swData1 & 256) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 23:  if (swData1 & 512) ++testCheckMode;
                    else testCheckMode = 0;
                    break;
          case 24:  if (swData1 & 16384) {
...
if (bDelete == 0) OESprCreateIndx(kind);
...
if (swData1 & 53248) {
    ...
    if (point.x >= infoSprtBmp[kind].point.x) {
      ...
      if (GetNextMenu(menuEnable, kind - 25, 0) + 25 < kind) LArrow = 1;
      if (GetNextMenu(menuEnable, kind - 25, 1) + 25 > kind) RArrow = 1;
      point = infoSprtBmp[kind].point;
      OEClrset(tblPal2, 1);
      nSeqNo = 6;
      timeOld = nTimerCunt;
    ...
    else if (swData1 & 1024 && GetNextMenu(menuEnable, kind - 25, 0) + 25 < kind) {

timeOld = nTimerCunt;
      nSeqNo = 7;
      nSeqNoNext = 2;
    }
    else if (swData1 & 2048 && GetNextMenu(menuEnable, kind - 25, 1) + 25 > kind) {
...
timeOld = nTimerCunt;
  ...
  if (LArrow != 0) OESprCreateIndx(32);
  if (RArrow != 0) OESprCreateIndx(33);
  if (nSeqNo != 1) OESprCreateIndx(kind);

if (kPosi != 0) hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[kind].lphSpr], point.x, point.y);
  if (lPosi != 0) hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[32].lphSpr], arrowPoint.x, arrowPoint.y);
  if (rPosi != 0) hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[33].lphSpr], arrowPoint.x, arrowPoint.y);
}
...
void OEFadeOut(void) {
  ...
  if (nTimerCunt == 0) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (bFadePaletDisable[i] == 0) OEClrset(workPalet[i], i);
    }
  }
    ...
    if (nFadoValu * nTimerCunt < 256) {
      ...
      for (n = 0; n < 4; ++n) {
        ...
        switch (n) {
          ...
          case 0: lpPal = tblPal1; break;
          case 1: lpPal = tblPal2; break;
          case 2: lpPal = tblPal3; break;
          case 3: lpPal = tblPal4; break;
        }
        for (j = 0; j < 16; ++j) {
        ...
        if (bFadePaletDisable[n] == 0) OEClrset(workPalet[n], n);
      }
    }
  }
}
...
void OEOpenSta(void) {
  if (nTimerCunt == 0) {
    ...
    OEGridCreate(0);
    if (bFirstTitle != 0) OESetSequenceNum(2);
