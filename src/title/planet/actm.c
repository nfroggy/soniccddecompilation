...
static void pattern_chg(sprite_status_lpl *pActwk,
                        sprite_patterns_title *pPatTbl);

static void (*act_tbl[31])(sprite_status_lpl *) = {
    &flicky_ctrl,  &star_ctrl, &eggm_ctrl, &ufo_ctrl, &msnc_ctrl, &tails_ctrl,
    &ascii_sprite, &dummy,     &dummy,     &dummy,    &dummy,     &dummy,
...
static void pattern_chg(sprite_status_lpl *pActwk,
    ...
    if (--pActwk->SPR_TIMER < 0) {
        if ((PatNoWk = pActwk->PAT_NO + 1) >= pPatTbl->patnum)
            PatNoWk = 0;
