...
void spr_kill(sprite_status_lpl *pActwk) {
    actset_flg ^= (Uint8)pActwk->ACT_FLG;
    pActwk->SPR_FLG |= 16;
}
...
void spr_kills(sprite_status_lpl *pActwk) {
    ...
    pActwk->SPR_FLG |= 16;
    for (i = 0; i < 16; ++i) {
        if (actwk[i].ACT_NO == wD0)
            return;
    }
    actset_flg ^= (Uint8)pActwk->ACT_FLG;
}

Sint16 killchk(sprite_status_lpl *pActwk) {
...
Sint16 chk_another(Uint16 wD0, sprite_status_lpl *pActwk,
                   sprite_status_lpl **ppActwk) {
    Sint16 i;
    ...
    for (i = 0; i < 16; ++i, ++pActfind) {
        if (pActfind->ACT_NO == wD0)
            if (pActfind - actwk != pActwk - actwk) {
                *ppActwk = pActfind;
                return 1;
            }
    }
...
void fslowmove(sprite_status_lpl *pActwk) {
    ...
    if (chk_another(pActwk->ACT_NO, pActwk, &pActflicky) == 0)
        goto label1;
...
void ufo_ctrl(sprite_status_lpl *pActwk) { ufo_acttbl[pActwk->EXE_NO](pActwk); }
