...
if (gMenu1 == 0) {
        ...
        if (gNewMenuRound < 0) gNewMenuRound = GetPlayRound();
      }
      else if (swdata1.b.h & 2) {

        gMove = 7;
        gNewMenuRound = gMenuRound + 1;
        if (gNewMenuRound > GetPlayRound()) gNewMenuRound = 0;
      }
    }
...
if (c < 48) c = 90;
          ...
          if (gMenu1 == 0) {
            lpScoreData->timeattack[gMenuRound - 1][gMenuZone][gRankY].name[gRankX] = c;
          }
          else {
            lpScoreData->special[gMenuRound - 1][gRankY].name[gRankX] = c;
          }
          gMove = 12;
        }
      }
...
if (gMenu2 == 2) {
      ...
      if (gMove == 0) {
        if (swdata1.b.l & 4) {
          if (gMenu1 == 0) {
            if (gMenu2 == 2) gNewMenu2 = 1;
            else if (gMenu2 == 3) gNewMenu2 = 2;
          }
          else {
            if (gMenu2 == 1) gNewMenu2 = 3;
          }
          gMove = 10;
        }
        else if (swdata1.b.l & 8) {
          if (gMenu1 == 0) {
            if (gMenu2 == 1) gNewMenu2 = 2;
            else if (gMenu2 == 2) gNewMenu2 = 3;
          }
          else {
            if (gNewMenu2 == 3) gNewMenu2 = 1;
          }
          gMove = 10;
        }
      }
...
round = gMenuRound - 1;
      ...
      for (i = 0; i < 3; ++i) {
        if (gMenu1 == 0) {
          if (lpScoreData->timeattack[round][zone][i].time >= lpKeepWork->ta_time) break;

}
        else {
          if (lpScoreData->special[round][i].time >= lpKeepWork->ta_time) break;
      ...
      if (i != 3) {
        ...
        for (j = 2; j > i; --j) {
          ...
          if (gMenu1 == 0) {
            lpScoreData->timeattack[round][zone][j].time = lpScoreData->timeattack[round][zone][j - 1].time;
            sStrncpy(lpScoreData->timeattack[round][zone][j].name, lpScoreData->timeattack[round][zone][j - 1].name, 3);
          }
          else {
            lpScoreData->special[round][j].time = lpScoreData->special[round][j - 1].time;
            sStrncpy(lpScoreData->special[round][j].name, lpScoreData->special[round][j - 1].name, 3);
          }
        }

        if (gMenu1 == 0) {
          lpScoreData->timeattack[round][zone][i].time = lpKeepWork->ta_time;
          if (sStrncmp(lpScoreData->timeattack[round][zone][i].name, aNameDefault, 3) == 0) {
            sStrncpy(lpScoreData->timeattack[round][zone][i].name, aNamePlayer, 3);
          }
          ...
          if (sStrncmp(lpScoreData->special[round][i].name, aNameDefault, 3) == 0) {
            sStrncpy(lpScoreData->special[round][i].name, aNamePlayer, 3);
          }
        }
      }
    }
