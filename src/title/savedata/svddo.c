...
void DeleteFileSelect(void) { OESprDeleteIndx(192); }
...
void UpdateFileSelect(Uint16 nNewNumber) {
    ...
    hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[192].lphSpr],
                                   point.x, point.y);
}
...
void UpdateSelectSonicMark(Uint16 nNewNumber) {
    ...
    hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[10].lphSpr],
                                   point.x, point.y);
}
...
void DefaultMessage(void) { UpdateMessage("SONIC CD DATA-FILES"); }
