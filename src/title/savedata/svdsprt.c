...
switch (kind)
  {
    case 0:
      if (c >= 48 && c < 58) indx = c - 37;
      else if (c >= 65 && c < 91) indx = c - 44;
      else
      {
      ...
      break;
    case 1:
      if (c >= 48 && c < 58) indx = c - 1;
      else if (c >= 65 && c < 91) indx = c - 8;
      else
      {
        indx = GetMarkfontIndx(c, kind);
        if (indx == -1) return 0;
      ...
      break;
    case 2:
      if (c >= 48 && c < 58) indx = c + 35;
      else if (c >= 65 && c < 91) indx = c + 28;
      else
      {
        indx = GetMarkfontIndx(c, kind);
        if (indx == -1) return 0;
    ...
    case 3:
      if (c >= 48 && c < 58) indx = c + 71;
      else if (c >= 65 && c < 91) indx = c + 64;
      else
      {
        indx = GetMarkfontIndx(c, kind);
        if (indx == -1) return 0;
...
hmx_sprite_set_position_module(s_ctx->sprites[hSprMes[nSprNum]], point.x, point.y);
    hmx_sprite_set_bitmap_module(s_ctx->sprites[hSprMes[nSprNum]], s_ctx->spr_bitmaps[indx]);
...
hmx_sprite_set_position_module(s_ctx->sprites[hSprFile[nBlockNo - 2][nSprNum]], point.x, point.y);
    hmx_sprite_set_bitmap_module(s_ctx->sprites[hSprFile[nBlockNo - 2][nSprNum]], s_ctx->spr_bitmaps[indx]);
    hmx_sprite_set_flags_module(s_ctx->sprites[hSprFile[nBlockNo - 2][nSprNum]], 0);
    s_ctx->spr_level[hSprFile[nBlockNo - 2][nSprNum]] = PRIO_MAX - infoSprtBmp[indx].order;
    return 1;
  }
  return 0;
}
...
hmx_sprite_set_bitmap_module(s_ctx->sprites[hSprMes[nSprNum]], 0);
  ...
  else if (nBlockNo >= 2 || nBlockNo < 9)
  {
    ...
    if (hSprFile[nBlockNo - 2][nSprNum] != 0)
    {
      ...
      hmx_sprite_set_bitmap_module(s_ctx->sprites[hSprFile[nBlockNo - 2][nSprNum]], 0);
...
void MovCursol(Sint32 x) {
  ...
  hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[191].lphSpr], point.x, point.y);
}
...
void DispSonicCursol(POINT point) {
  ...
  hmx_sprite_set_position_module(s_ctx->sprites[*infoSprtBmp[nSonicKind].lphSpr], posiSonicCursol.x, posiSonicCursol.y);
  ...
  if (bMenuChosed == 0)
  {
    hmx_sprite_set_flags_module(s_ctx->sprites[*infoSprtBmp[nSonicKind].lphSpr], 1);

  }
  else
  {
    hmx_sprite_set_flags_module(s_ctx->sprites[*infoSprtBmp[nSonicKind].lphSpr], 0);
...
static void MovSonicCursol(POINT point) {
  ...
  if (nSonicKind == 203
      ...
      || nSonicKind == 205)
    nSonicKind = 206;
  else if (nSonicKind == 208) nSonicKind = 206;
  else ++nSonicKind;
  OESprCreateIndx(nSonicKind);
  posiSonicCursol = point;
...
void RunSonicCursol(POINT point) {
  ...
  if (sx.w.h == 0) fSonicIncX = 0;
  else fSonicIncX = SONICMOVEM_MAG * (sx.l / isqrt((sx.w.h * sx.w.h + sy.w.h * sy.w.h) * 65536));
  if (sy.w.h == 0) fSonicIncY = 0;
  else fSonicIncY = SONICMOVEM_MAG * (sy.l / isqrt((sx.w.h * sx.w.h + sy.w.h * sy.w.h) * 65536));
...
void UpdateSonicCursol(void) {
  if (nSonicMode == 1)
  {
    if (nTimerCunt % 20 != 0) return;

    if (nTimerCunt - nStandStartTimerCunt <= 60) return;

    HideSonicCursol();
    if (nSonicKind == 203) nSonicKind = 204;
    else if (nSonicKind == 204) nSonicKind = 205;
    else if (nSonicKind == 205) nSonicKind = 204;
    else nSonicKind = 203;
    OESprCreateIndx(nSonicKind);
...
posiSonicCursol.x = posiStartXOld.w.h;
    ...
    if (posiSonicCursol.x == posiTarget.x && posiSonicCursol.y == posiTarget.y)
    {
      HideSonicCursol();
