...
void SDMenuCopy(void) {
    ...
    if (bFileChosed == 0) {
        ...
        if (swData1 & 16384) {
            ...
            WaveRequest(5);
            sMemCpy(&saveData[nFileNumber], &saveData[nSrcFileNumber],
                    sizeof(saveData[nSrcFileNumber]));

            bMenuChosed = 0;
            UpdateFileView(nFileNumber);
            UpdateMessage("COPY COMPLETED.");
            DeleteFileSelect();
            point.x = infoSprtBmp[2].point.x + infoSprtBmp[2].w;
...
void SDMenuRename(void) {
    ...
    if (bFileChosed == 0) {
        ...
        if (saveData[nFileNumber].saved != 0) {
            ...
            sMemCpy(editBuf, saveData[nFileNumber].player,
                    sizeof(((score_data *)0)->player));
            DispCursol();
            MovCursol(0);
        } else {
            WaveRequest(30);
        ...
        if (ret == 1 || ret == -1) {
            ...
            WaveRequest(5);
            sMemCpy(saveData[nFileNumber].player, editBuf,
                    sizeof(((score_data *)0)->player));
            bMenuChosed = bFileChosed = editMode = 0;
            DefaultMessage();
            DeleteFileSelect();
            HideCursol();
            point.x = infoSprtBmp[4].point.x + infoSprtBmp[4].w;
...
void SDMenuDelete(void) {
    ...
    if (bFileChosed == 0) {
        ...
        if (swData1 & 16384) {
            ...
            DeleteFileSelect();
            point.x = infoSprtBmp[6].point.x + infoSprtBmp[6].w;
...
void SDMenuExit(void) { OESetSequenceNum(999); }
