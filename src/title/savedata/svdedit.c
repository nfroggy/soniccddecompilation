...
Sint32 Edit(void) {
    ...
    if (swData1 & 1024) {
        ...
        if (editBuf[posi] == 32)
            editBuf[posi] = 90;
        else if (editBuf[posi] == 45)
            editBuf[posi] = 32;
        else if (editBuf[posi] == 48)
            editBuf[posi] = 45;
        else if (editBuf[posi] == 65)
            editBuf[posi] = 57;
        else
            --editBuf[posi];
        ...
        if (editBuf[posi] == 32)
            ...
            editBuf[posi] = 48;
        else if (editBuf[posi] == 57)
            editBuf[posi] = 65;
        else if (editBuf[posi] == 90)
            editBuf[posi] = 32;
        else
            ++editBuf[posi];
        DeleteCharSprt(nFileNumber + 2, posi);
...
void DispSaveData(Sint32 indx) {
    ...
    if (saveData[indx].saved != 0) {
        ...
        sPrintf(str, "%4u/%2u/%2u", saveData[indx].year, saveData[indx].month,
                saveData[indx].date);
        ...
        sPrintf(str, "%02u\"%02u\"%02u", saveData[indx].hour,
                saveData[indx].min, saveData[indx].sec);
