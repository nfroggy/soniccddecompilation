...
void action(void) {
  ...
  for (i = 0; i < 14; ++i) actmain(&actwk[i + 2]);

  actmain(&actwk[0]);
  actmain(&actwk[1]);

  for (i = 0; i < 24; ++i) actmain(&actwk[i + 16]);

zbuf_patset();

  for (i = 0; i < 8; ++i) actmain(&actwk[i + 40]);
  ...
  for (i = 0; i < sprpat_adr->cnt; ++i) {
    ...
    switch (sprdat.etc & 24) {
      ...
      case 8:
        y_posi = sy + (Sint16)sprdat.yoff;
        x_posi = sx - (Sint16)sprdat.xoff
                    - (Sint16)SprBmp[sprdat.index].xs;
        rev = 1;
        break;
      case 16:
        y_posi = sy - (Sint16)sprdat.yoff
                    - (Sint16)SprBmp[sprdat.index].ys;
        x_posi = sx + (Sint16)sprdat.xoff;
        rev = 2;
        break;
      case 24:
    ...
    if (y_posi + (Sint16)SprBmp[sprdat.index].ys > 128
        && y_posi < 384
        && x_posi + (Sint16)SprBmp[sprdat.index].xs > 128
        && x_posi < 448) {
