...
static Uint16 chibi_sonic_map[2][3][2] = {
  {
    { 863, 864 },
    { 869, 870 },
    { 871, 872 }
...
void sonic_disp_disp(Sint16 tbl_no) {
  ...
  for (y = 0; y < 3; ++y) {
    for (x = 0; x < 2; ++x) {
      SetGrid(0, x + 26 + tbl_no, y + 23, chibi_sonic_map[sonic_upcnt][y][x], 0);
...
Uint16 cntminus(Uint16* work, Uint16 minus_data, Uint16 under_limit, Uint16 upper_limit) {
  if ((Sint16)(*work -= minus_data) < (Sint16)under_limit) *work = upper_limit;
  return *work;
}
