...
void playposiset(void) {
  ...
  if (plflag) {
    ...
    d1 = *a1++;
    actwk[0].xposi.w.h = d1;
    d0 = *a1;
    actwk[0].yposi.w.h = d0;
  }

  if ((Uint16)d1 >= 160) d1 -= 160;
  else d1 = 0;
  if ((Uint16)d1 >= (Uint16)scralim_right) d1 = scralim_right;
  scra_h_posit.w.h = d1;

  if ((Uint16)d0 >= 96) d0 -= 96;
  else d0 = 0;
  if (d0 >= scralim_down) d0 = scralim_down;
  scra_v_posit.w.h = d0;
...
void scrbinit(Sint16 d1, Sint16 d0) {
  int_union ld0;

ld0.w.h = d0;
  ld0.w.l = 0;

  ld0.l = (long int)(ld0.l >> 4) * 3;
  scrb_v_posit.l = ld0.l;
  scrc_v_posit.w.h = (Uint16)(d0 / 16) * 2;
  scrz_v_posit.w.h = (Uint16)(d0 / 16) * 2;

  scrc_h_posit.w.h = (Uint16)(d1 / 4);
  scrz_h_posit.w.h = (Uint16)(d1 / 8);
  scrb_h_posit.w.h = (Uint16)(d1 / 16 * 3);
...
void scroll(void) {
  ...
  Sint16 *p, *pHscrollwork;
  Sint16 s;
  Sint32 i, j;
  Sint32 cnt, temp;
  static Sint32 hs[3] = { 65536, 49152, 32768 };
...
scrb_h_count ^= 16;
    lD0.l -= lD2.l;
    if ((long int)lD0.l < 0) scrflagb.b.h |= flagb;
    else scrflagb.b.h |= flagb * 2;
  }
}
...
void hblockwrt(POINT* pTilePoint, Sint32 VramBase, Uint16 wH_posiw, Uint16 wV_posiw, Uint16 xOffs, Uint16 yOffs, Uint8* pMapWk, Sint32 lpcnt) {
  ...
  do {
    ...
    pTilePoint->x += 2;
    if (pTilePoint->x >= 64) pTilePoint->x -= 64;
    xOffs += 16;
  } while (--lpcnt >= 0);
}
...
void mapwrt_z11a(Uint8* pMapWk, Sint32 VramBase) {
  ...
  do {
    ...
    mapwrt_sub(&z12awrttbl[1], wD0, wD4, pMapWk, VramBase);
    wD4 += 16;
  } while ((Sint16)--wD6 >= 0);
}
