...
static Uint8 z11bwrttbl[33] = {0, 0, 0, 0, 0, 6, 6, 6, 6, 6, 4,
...
void playposiset(void) {
    ...
    if ((Uint16)yWk > 96)
        ...
        yWk = 0;
    if ((Uint16)scralim_down < (Uint16)yWk)
        yWk = scralim_down;
...
void scroll(void) {
    ...
    for (i = 0; i < 2; ++i)
        *pHScrollWork++ = -(hscrollwork[1] + scrz_h_posit.w.h);
    for (i = 0; i < 2; ++i)
        *pHScrollWork++ = -(hscrollwork[3] + scrz_h_posit.w.h);
    for (i = 0; i < 2; ++i)
        *pHScrollWork++ = -(hscrollwork[5] + scrz_h_posit.w.h);
    for (i = 0; i < 2; ++i)
        *pHScrollWork++ = -(hscrollwork[7] + scrz_h_posit.w.h);

    for (i = 0; i < 10; ++i)
        *pHScrollWork++ = -scrz_h_posit.w.h;
    for (i = 0; i < 24; ++i)
...
void mapwrt_z11b(Uint8 *pMapWk, Sint32 VramBase) {
    Uint16 wD0, wD4, wD6;
    ...
    do {
        ...
        mapwrt_sub(&z11bwrttbl[1], wD0, wD4, pMapWk, VramBase);
        wD4 += 16;
    } while ((Sint16)--wD6 >= 0);
}
