...
#include "src/r8/dev81c.c"

static void reset_dev81c_state(void) {
...
static void test_dev81c_map_init_data_captures_literal_values(
    test_context *ctx) {
...
static void test_dev81c_tables_capture_representative_literal_values(
    test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 360, tbl_pis.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 368, tbl_com.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 376, tbl_mot.TileStart);
}

static void test_dev81c_cg_chg1_waits_while_timer_remains(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[16] = {-1};
    Sint32 tile_start = -1;
...
static void test_dev81c_cg_chg1_advances_to_next_frame(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[8];
    Sint32 tile_start = -1;
    ...
    TEST_ASSERT_TRUE(ctx, tim == &cgchgtim[1]);
    TEST_ASSERT_TRUE(ctx, cnt == &cgchgcnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 360, tile_start);
    for (int i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 8 + i, bmp_no[i]);
    }
}

static void test_dev81c_cg_chg1_wraps_to_first_frame(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[16];
    Sint32 tile_start = -1;
    ...
    for (int i = 0; i < 16; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 56 + i, bmp_no[i]);
    }
}

static void test_dev81c_cg_change_wait_path_makes_no_tile_calls(
    test_context *ctx) {
    reset_dev81c_state();
    for (int i = 0; i < 3; ++i) {
        cgchgtim[i] = 2;
...
static void test_dev81c_cg_change_emits_exact_tile_sequence(test_context *ctx) {
    reset_dev81c_state();
    cgchgtim[0] = 0;
    cgchgcnt[0] = 0;
    cgchgtim[1] = 0;
    ...
    assert_tile_range(ctx, 0, 360, 8, 8);
    assert_tile_range(ctx, 8, 368, 40, 8);
    assert_tile_range(ctx, 16, 376, 104, 16);
}

TEST_MAIN_BEGIN;
    test_dev81c_map_init_data_captures_literal_values(&ctx);
    test_dev81c_tables_capture_representative_literal_values(&ctx);
    test_dev81c_cg_chg1_waits_while_timer_remains(&ctx);
    test_dev81c_cg_chg1_advances_to_next_frame(&ctx);
    test_dev81c_cg_chg1_wraps_to_first_frame(&ctx);
    test_dev81c_cg_change_wait_path_makes_no_tile_calls(&ctx);
    test_dev81c_cg_change_emits_exact_tile_sequence(&ctx);
TEST_MAIN_END
