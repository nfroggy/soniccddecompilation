...
#include "src/r8/col8b.c"

static void reset_col8b_state(void) {
...
static void test_col8b_tables_capture_palette_contract(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone8colorb) / sizeof(zone8colorb[0])));
    TEST_ASSERT_EQ_INT(ctx, 6, (int)(sizeof(colortbl) / sizeof(colortbl[0])));

    assert_palette(ctx, advacolor[0], 0, 32, 160, 1, "advacolor[0]");
    assert_palette(ctx, advacolor2[49], 192, 224, 224, 1, "advacolor2[49]");
    assert_palette(ctx, gamecolor[0], 96, 32, 128, 1, "gamecolor[0]");
    assert_palette(ctx, zone8colora[16], 96, 32, 128, 1, "zone8colora[16]");
    assert_palette(ctx, zone8colora[47], 0, 224, 0, 1, "zone8colora[47]");
    assert_palette(ctx, zone8colorb[16], 64, 0, 160, 1, "zone8colorb[16]");
    assert_palette(ctx, zone8colorb[47], 224, 224, 224, 1,
                   "zone8colorb[47]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == zone8colorb);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 48, colortbl[5].cnt);

    TEST_ASSERT_EQ_INT(ctx, 38, col8a0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 62, col8a3cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 49, col8a5cnt[0]);
    assert_palette(ctx, col8a0col[0], 224, 160, 96, 1, "col8a0col[0]");
    assert_palette(ctx, col8a3col[2], 0, 0, 0, 1, "col8a3col[2]");
    assert_palette(ctx, col8a6col[0], 224, 224, 224, 1, "col8a6col[0]");
}
...
static void test_colchg8b_updates_normal_and_stage_one_cycles(
    test_context *ctx) {
    reset_col8b_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 15, clchgtim[4]);
    assert_palette(ctx, colorwk[38], 192, 96, 64, 1, "cycle color 38");
    assert_palette(ctx, colorwk[39], 160, 32, 32, 1, "cycle color 39");
    assert_palette(ctx, colorwk[40], 224, 160, 96, 1, "cycle color 40");
    assert_palette(ctx, colorwk[62], 224, 224, 224, 1,
                   "normal cycle color 62");
    assert_palette(ctx, colorwk[63], 224, 224, 0, 1,
                   "normal cycle color 63");
    ...
    assert_palette(ctx, colorwk[49], 224, 224, 224, 1,
                   "stage1 cycle color 49");
    assert_palette(ctx, colorwk[50], 0, 0, 0, 1, "stage1 cycle color 50");
}
...
static void test_flash_remaining_completion_and_stage_skip_paths(
    test_context *ctx) {
    reset_col8b_state();
    flashin();
    FadeCount = 21;
    ...
    colorwk3[0] = (PALETTEENTRY){224, 224, 224, 7};
    colorwk4[0] = (PALETTEENTRY){0, 0, 0, 1};

    TEST_ASSERT_EQ_INT(ctx, 0, flashin_new());
    assert_palette(ctx, colorwk[0], 224, 224, 192, 1,
                   "flashin stage skip work 0");
    assert_palette(ctx, colorwk3[0], 224, 224, 224, 7,
                   "flashin stage skip work 2");
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    colorset(3);
    assert_palette(ctx, colorwk2[0], 96, 32, 128, 1, "colorset work 1");
...
TEST_MAIN_BEGIN;
test_col8b_tables_capture_palette_contract(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg8b_updates_normal_and_stage_one_cycles(&ctx);
test_fadein_initializes_range_and_steps_toward_sources(&ctx);
test_fadeout_flash_and_direct_helpers(&ctx);
test_flash_remaining_completion_and_stage_skip_paths(&ctx);
