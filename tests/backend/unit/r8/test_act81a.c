...
DEFINE_ACTOR_STUB(propera)
DEFINE_ACTOR_STUB(tobira)
DEFINE_ACTOR_STUB(konbea)
DEFINE_ACTOR_STUB(dai8)
DEFINE_ACTOR_STUB(hachi8)
DEFINE_ACTOR_STUB(scarab)
DEFINE_ACTOR_STUB(friend)
DEFINE_ACTOR_STUB(piston)
DEFINE_ACTOR_STUB(okusieso)
DEFINE_ACTOR_STUB(togedair8)
DEFINE_ACTOR_STUB(anar8)
DEFINE_ACTOR_STUB(futagor8)
...
static void test_act81a_table_captures_exact_actor_routing(test_context *ctx) {
    ...
    assert_act_entry(ctx, 39, propera);
    ...
    assert_act_entry(ctx, 45, hachi8);
    assert_act_entry(ctx, 46, scarab);
    ...
    assert_act_entry(ctx, 52, okusieso);
    assert_act_entry(ctx, 53, togedair8);
    assert_act_entry(ctx, 54, anar8);
    assert_act_entry(ctx, 55, futagor8);
    assert_act_entry(ctx, 56, noact);
    assert_act_entry(ctx, 57, clear);
    assert_act_entry(ctx, 58, over);
    assert_act_entry(ctx, 59, title);
    assert_act_entry(ctx, 60, noact);
    assert_act_entry(ctx, 61, noact);
    assert_act_entry(ctx, 62, noact);
    assert_act_entry(ctx, 63, noact);
    assert_act_entry(ctx, 64, noact);
    assert_act_entry(ctx, 65, noact);
    assert_act_entry(ctx, 66, et);
}
...
TEST_MAIN_BEGIN;
    test_act81a_table_captures_exact_actor_routing(&ctx);
