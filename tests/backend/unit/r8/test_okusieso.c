...
static void test_okusieso_initializes_parent_and_children(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, parent->actfree[6]);
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, ball->actfree[20]);
    ...
    TEST_ASSERT_EQ_INT(ctx, 2, platform->actfree[20]);
...
static void test_okusieso_ball_launches_then_lands(test_context *ctx) {
    ...
    TEST_ASSERT_TRUE(ctx, (ball->actfree[20] & 64) != 0);
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, ball->yspeed.w);
    TEST_ASSERT_TRUE(ctx, (ball->actfree[20] & 8) != 0);
    TEST_ASSERT_TRUE(ctx, (parent->actfree[20] & 8) != 0);
...
static void test_okusieso_platform_ride_triggers_parent_and_tracks_patno(
    ...
    TEST_ASSERT_TRUE(ctx, (parent->actfree[20] & 16) != 0);
    TEST_ASSERT_TRUE(ctx, (parent->actfree[20] & 4) != 0);
...
static void test_okusieso_parent_remaining_early_return_paths(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, parent->patno);
    TEST_ASSERT_EQ_INT(ctx, 1, parent->actfree[7]);
    assert_action_only(ctx, parent);
}
...
static void test_okusieso_parent_animation_timer_phases(test_context *ctx) {
    ...
    TEST_ASSERT_TRUE(ctx, (parent->actfree[20] & 64) != 0);
    TEST_ASSERT_EQ_INT(ctx, 1, parent->patno);
    TEST_ASSERT_EQ_INT(ctx, 5, parent->actfree[7]);
    assert_action_only(ctx, parent);
    ...
    TEST_ASSERT_EQ_INT(ctx, 2, parent->patno);
    TEST_ASSERT_EQ_INT(ctx, 5, parent->actfree[7]);
    TEST_ASSERT_TRUE(ctx, (parent->actfree[20] & 32) == 0);
    assert_action_only(ctx, parent);
    ...
    TEST_ASSERT_EQ_INT(ctx, 4, parent->patno);
    TEST_ASSERT_TRUE(ctx, (parent->actfree[20] & 32) != 0);
    TEST_ASSERT_EQ_INT(ctx, 0, parent->actfree[6]);
    TEST_ASSERT_EQ_INT(ctx, 10, parent->actfree[7]);
    assert_action_only(ctx, parent);

    reset_logs();
    parent->actfree[20] &= (Uint8)~4;
    parent->actfree[7] = 0;
    parent->actfree[6] = 0;
