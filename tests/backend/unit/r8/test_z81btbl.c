...
static void test_z81btbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 603607584u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 3673633111u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 300892413u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z81btbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z81btbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 75, mapwka[1][27]);
...
TEST_MAIN_BEGIN;
    test_z81btbl_table_sizes_and_fingerprints(&ctx);
    test_z81btbl_direction_and_collision_anchors(&ctx);
    test_z81btbl_map_anchors(&ctx);
TEST_MAIN_END
