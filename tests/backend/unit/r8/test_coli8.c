...
static void reset_coli8_state(void) {
...
static void test_coli8_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 15, colitbl[56][0]);
    TEST_ASSERT_EQ_INT(ctx, 32, colitbl[56][1]);
    TEST_ASSERT_EQ_INT(ctx, 32, colitbl[57][0]);
    TEST_ASSERT_EQ_INT(ctx, 15, colitbl[57][1]);
    TEST_ASSERT_EQ_INT(ctx, 32, colitbl[62][0]);
    TEST_ASSERT_EQ_INT(ctx, 8, colitbl[62][1]);
...
static void test_collision_table_x_and_y_bounds(test_context *ctx) {
    ...
    target->colino = 65;
    target->yposi.w.h = 100;
    TEST_ASSERT_EQ_INT(ctx, 0, CollitblDataYchk(player, target, 30, 0, 0, 16));

    reset_coli8_state();
    target->colino = 65;
    target->yposi.w.h = 100;
    TEST_ASSERT_EQ_INT(ctx, 0,
                       CollitblDataYchk(player, target, 130, 0, 0, 16));

    reset_coli8_state();
    target->colino = 65;
    target->yposi.w.h = 100;
    TEST_ASSERT_EQ_INT(ctx, 0, CollitblDataYchk(player, target, 60, 0, 0, 16));
...
static void test_hit_dispatch_routes_by_colino_high_bits(test_context *ctx) {
    ...
    plpower_m = 1;
    target->colino = 129;
    TEST_ASSERT_EQ_INT(ctx, 0, ColliHitChk(player, target, 0, 0, 16));
...
static void test_special_boss_and_yago_collision_paths(test_context *ctx) {
    sprite_status *player = &actwk[0];
    ...
    TEST_ASSERT_EQ_INT(ctx, -384, boss->yspeed.w);
    ...
    plpower_m = 1;
    boss->xposi.w.h = 100;
    TEST_ASSERT_EQ_INT(ctx, -1, yago(player, boss, 120, 0, 7));
    TEST_ASSERT_EQ_INT(ctx, 0, jumpcolsub_count);
    ...
    plpower_m = 1;
    boss->xposi.w.h = 100;
    TEST_ASSERT_EQ_INT(ctx, -1, yago(player, boss, 60, 0, 7));
    TEST_ASSERT_EQ_INT(ctx, 0, jumpcolsub_count);
...
TEST_MAIN_BEGIN;
test_coli8_tables_capture_literal_data(&ctx);
...
test_special_boss_and_yago_collision_paths(&ctx);
