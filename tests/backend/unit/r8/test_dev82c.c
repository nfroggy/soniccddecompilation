...
#include "src/r8/dev82c.c"

static void reset_dev82c_state(void) {
...
static void test_dev82c_map_init_data_captures_literal_values(
    test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 6, mapinittbl.colorno1);
    TEST_ASSERT_EQ_INT(ctx, 6, mapinittbl.colorno2);
    TEST_ASSERT_EQ_INT(ctx, 129, mapinittbl.soundNo);
}

static void test_dev82c_tables_capture_representative_literal_values(
    test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 422, tbl_pis.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 430, tbl_com.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 438, tbl_mot.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 458, tbl_pan.TileStart);
}

static void test_dev82c_cg_chg1_waits_while_timer_remains(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[16] = {-1};
    Sint32 tile_start = -1;
...
static void test_dev82c_cg_chg1_advances_to_next_frame(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[8];
    Sint32 tile_start = -1;
    ...
    TEST_ASSERT_TRUE(ctx, tim == &cgchgtim[1]);
    TEST_ASSERT_TRUE(ctx, cnt == &cgchgcnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 422, tile_start);
    for (int i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 8 + i, bmp_no[i]);
    }
}

static void test_dev82c_cg_chg1_wraps_to_first_frame(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[4];
    Sint32 tile_start = -1;
    ...
    TEST_ASSERT_EQ_INT(ctx, 458, tile_start);
...
static void test_dev82c_cg_change_wait_path_makes_no_tile_calls(
    test_context *ctx) {
    reset_dev82c_state();
    for (int i = 0; i < 4; ++i) {
        cgchgtim[i] = 2;
...
static void test_dev82c_cg_change_emits_exact_tile_sequence(test_context *ctx) {
    reset_dev82c_state();
    cgchgtim[0] = 0;
    cgchgcnt[0] = 0;
    cgchgtim[1] = 0;
    ...
    assert_tile_range(ctx, 0, 422, 8, 8);
    assert_tile_range(ctx, 8, 430, 40, 8);
    assert_tile_range(ctx, 16, 438, 104, 16);
    assert_tile_range(ctx, 32, 458, 120, 4);
}

TEST_MAIN_BEGIN;
    test_dev82c_map_init_data_captures_literal_values(&ctx);
    test_dev82c_tables_capture_representative_literal_values(&ctx);
    test_dev82c_cg_chg1_waits_while_timer_remains(&ctx);
    test_dev82c_cg_chg1_advances_to_next_frame(&ctx);
    test_dev82c_cg_chg1_wraps_to_first_frame(&ctx);
    test_dev82c_cg_change_wait_path_makes_no_tile_calls(&ctx);
    test_dev82c_cg_change_emits_exact_tile_sequence(&ctx);
TEST_MAIN_END
