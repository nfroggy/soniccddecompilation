...
static void reset_scr82a_state(void) {
...
static void test_scr82a_main_chk_selects_active_player(test_context *ctx) {
    reset_scr82a_state();
...
static void test_scr82a_scr_set_uses_default_start_and_parallax(
    test_context *ctx) {
    reset_scr82a_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 46, actwk[0].xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 1164, actwk[0].yposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 0, scra_h_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 1068, scra_v_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 133, scrb_v_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 133, scrc_v_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 133, scrz_v_posit.w.h);
...
static void test_scr82a_scr_set_loads_saved_position_when_plflag_is_set(
    test_context *ctx) {
    reset_scr82a_state();
    plflag = 1;
...
static void test_scr82a_scr_set_uses_keep_work_demo_position(
    test_context *ctx) {
    reset_scr82a_state();
    keep_work.demoflag.w = 1;

    scr_set();

    TEST_ASSERT_EQ_INT(ctx, 6942, actwk[0].xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 1484, actwk[0].yposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 6782, scra_h_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 1388, scra_v_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 1269, scrb_h_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 173, scrb_v_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 0, playload_count);
}

static void test_scr82a_mapadrset_empty_and_populated_screens(
    test_context *ctx) {
    Sint32 index = 123;
...
static void test_scr82a_vram_address_masks_to_tile_grid(test_context *ctx) {
    POINT tile = {0, 0};
...
static void test_scr82a_blockwrt_emits_unflipped_and_flipped_quads(
    test_context *ctx) {
    POINT tile = {4, 6};
...
static void test_scr82a_blockwrt_emits_single_axis_flips(test_context *ctx) {
    POINT tile = {8, 10};
...
static void test_scr82a_scroll_respects_start_guard(test_context *ctx) {
    reset_scr82a_state();
    scroll_start.b.h = 1;
    scrflaga.w = 0x1234;
...
static void test_scr82a_scroll_moves_right_and_builds_hscroll(
    test_context *ctx) {
    reset_scr82a_state();
    scr_set();
    ...
    TEST_ASSERT_EQ_INT(ctx, 133, scrb_v_posit.w.h);
    TEST_ASSERT_EQ_INT(ctx, 133, vscroll.w.l);
}

static void test_scr82a_scroll_moves_left_and_flags_horizontal_edge(
    test_context *ctx) {
    reset_scr82a_state();
    scralim_left = 0;
    scralim_right = 7831;
...
static void test_scr82a_scroll_h_clamps_to_right_limit(test_context *ctx) {
    reset_scr82a_state();
    scralim_left = 0;
    scralim_right = 120;
    scra_hline = 160;
...
static void test_scr82a_vertical_move_paths_and_limits(test_context *ctx) {
    reset_scr82a_state();
    scralim_up = 0;
    scralim_down = 1808;
    scra_vline = 96;
...
static void test_scr82a_vertical_special_posture_paths(test_context *ctx) {
    reset_scr82a_state();
    scralim_up = 0;
    scralim_down = 1808;
    scra_vline = 96;
...
static void test_scr82a_scroll_wraps_vertical_edges(test_context *ctx) {
    int_union lSpd;
...
static void test_scr82a_scrollwrt_processes_all_screen_flags(
    test_context *ctx) {
    reset_scr82a_state();
...
static void test_scr82a_scrollwrtb_zero_row_uses_full_width_write(
    test_context *ctx) {
    Uint8 flag;
...
static void test_scr82a_mapwrt_and_block_wrt_emit_grid_updates(
    test_context *ctx) {
    reset_scr82a_state();
    seed_screen_zero((Uint8 *)mapwka);
    seed_screen_zero((Uint8 *)mapwkb);
...
#define SCR82_RESET reset_scr82a_state
#include "test_scr82_common.inc"
#undef SCR82_RESET

static void test_scr82a_mapinit_sets_primary_and_secondary_colors(
    test_context *ctx) {
    reset_scr82a_state();
...
TEST_MAIN_BEGIN;
test_scr82a_main_chk_selects_active_player(&ctx);
test_scr82a_scr_set_uses_default_start_and_parallax(&ctx);
test_scr82a_scr_set_loads_saved_position_when_plflag_is_set(&ctx);
test_scr82_scr_set_saved_position_clamps_to_scroll_limits(&ctx);
test_scr82a_scr_set_uses_keep_work_demo_position(&ctx);
test_scr82a_mapadrset_empty_and_populated_screens(&ctx);
test_scr82_mapadrset_clamps_high_offsets(&ctx);
test_scr82_mapadrset2_reports_block_pointer_and_empty_screen(&ctx);
test_scr82a_vram_address_masks_to_tile_grid(&ctx);
test_scr82_vram_address_extra_entry_points(&ctx);
test_scr82a_blockwrt_emits_unflipped_and_flipped_quads(&ctx);
test_scr82a_blockwrt_emits_single_axis_flips(&ctx);
test_scr82a_scroll_respects_start_guard(&ctx);
test_scr82a_scroll_moves_right_and_builds_hscroll(&ctx);
test_scr82a_scroll_moves_left_and_flags_horizontal_edge(&ctx);
test_scr82_scroll_h_exact_center_and_left_limit(&ctx);
test_scr82a_scroll_h_clamps_to_right_limit(&ctx);
test_scr82_scroll_negative_parallax_flags(&ctx);
test_scr82_scrollb_v_flags_up_and_down_edges(&ctx);
test_scr82a_vertical_move_paths_and_limits(&ctx);
test_scr82_vertical_edge_branches(&ctx);
test_scr82_direct_vertical_helpers_cover_small_moves(&ctx);
test_scr82a_vertical_special_posture_paths(&ctx);
test_scr82a_scroll_wraps_vertical_edges(&ctx);
test_scr82a_scrollwrt_processes_all_screen_flags(&ctx);
test_scr82a_scrollwrtb_zero_row_uses_full_width_write(&ctx);
test_scr82_scrollwrtb_bit2_and_empty_paths(&ctx);
test_scr82a_mapwrt_and_block_wrt_emit_grid_updates(&ctx);
test_scr82_block_wrt_visible_region_emits_grid_update(&ctx);
test_scr82a_mapinit_sets_primary_and_secondary_colors(&ctx);
