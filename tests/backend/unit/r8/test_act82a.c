...
static void test_act82a_table_captures_exact_actor_routing(test_context *ctx) {
    ...
    assert_act_entry(ctx, 44, dango);
    assert_act_entry(ctx, 45, noact);
    assert_act_entry(ctx, 46, noact);
    assert_act_entry(ctx, 47, gater8);
...
TEST_MAIN_BEGIN;
    test_act82a_table_captures_exact_actor_routing(&ctx);
