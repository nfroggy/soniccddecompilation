...
static void test_z83ctbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 2396665327u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 320333707u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 3826621632u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z83ctbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z83ctbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 43, mapwka[0][0]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwka[0][24]);
    TEST_ASSERT_EQ_INT(ctx, 193, mapwka[1][9]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwka[1][26]);
    TEST_ASSERT_EQ_INT(ctx, 1, mapwka[2][8]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwka[7][63]);

    TEST_ASSERT_EQ_INT(ctx, 103, mapwkb[0][0]);
...
TEST_MAIN_BEGIN;
    test_z83ctbl_table_sizes_and_fingerprints(&ctx);
    test_z83ctbl_direction_and_collision_anchors(&ctx);
    test_z83ctbl_map_anchors(&ctx);
TEST_MAIN_END
