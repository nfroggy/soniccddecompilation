...
static void test_z82ctbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 4113083235u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 2248926915u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 2567895165u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z82ctbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z82ctbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 51, mapwka[0][0]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwka[0][24]);
    TEST_ASSERT_EQ_INT(ctx, 60, mapwka[1][9]);
    TEST_ASSERT_EQ_INT(ctx, 95, mapwka[1][26]);
    TEST_ASSERT_EQ_INT(ctx, 60, mapwka[2][8]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwka[7][63]);

    TEST_ASSERT_EQ_INT(ctx, 100, mapwkb[0][0]);
    TEST_ASSERT_EQ_INT(ctx, 104, mapwkb[1][1]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwkb[2][0]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwkb[2][3]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwkb[7][63]);
}

TEST_MAIN_BEGIN;
    test_z82ctbl_table_sizes_and_fingerprints(&ctx);
    test_z82ctbl_direction_and_collision_anchors(&ctx);
    test_z82ctbl_map_anchors(&ctx);
TEST_MAIN_END
