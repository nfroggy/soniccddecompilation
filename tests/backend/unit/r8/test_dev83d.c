...
static tile_call tile_calls[48];
...
#include "src/r8/dev83d.c"

static void reset_dev83d_state(void) {
...
static void test_dev83d_map_init_data_captures_literal_values(
    test_context *ctx) {
...
static void test_dev83d_tables_capture_representative_literal_values(
    test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 148, pan1cg[0]);
    TEST_ASSERT_EQ_INT(ctx, 171, pan6cg[3]);
    ...
    TEST_ASSERT_EQ_INT(ctx, 302, tbl_pis.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 310, tbl_com.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 318, tbl_mot.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 338, tbl_spo.TileStart);
    ...
    TEST_ASSERT_EQ_INT(ctx, 334, tbl_pan.TileStart);
}

static void test_dev83d_cg_chg1_waits_while_timer_remains(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[16] = {-1};
    Sint32 tile_start = -1;
...
static void test_dev83d_cg_chg1_advances_to_next_frame(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[8];
    Sint32 tile_start = -1;
    ...
    TEST_ASSERT_TRUE(ctx, tim == &cgchgtim[1]);
    TEST_ASSERT_TRUE(ctx, cnt == &cgchgcnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 302, tile_start);
    for (int i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 8 + i, bmp_no[i]);
    }
}

static void test_dev83d_cg_chg1_wraps_to_first_frame(test_context *ctx) {
    Uint8 *tim;
    Uint8 *cnt;
    Sint32 bmp_no[4];
    Sint32 tile_start = -1;
    ...
    TEST_ASSERT_EQ_INT(ctx, 334, tile_start);
    TEST_ASSERT_EQ_INT(ctx, 148, bmp_no[0]);
    TEST_ASSERT_EQ_INT(ctx, 149, bmp_no[1]);
    TEST_ASSERT_EQ_INT(ctx, 150, bmp_no[2]);
    TEST_ASSERT_EQ_INT(ctx, 151, bmp_no[3]);
}

static void test_dev83d_cg_change_skips_pan_when_shut_flag_set_and_pis_count_zero(
    test_context *ctx) {
    reset_dev83d_state();
    shut_flag = 1;
    cgchgtim[0] = 2;
    ...
    TEST_ASSERT_EQ_INT(ctx, 28, tile_call_count);
    TEST_ASSERT_EQ_INT(ctx, 1, cgchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 0, cgchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 0, cgchgtim[4]);
    TEST_ASSERT_EQ_INT(ctx, 5, cgchgcnt[4]);
}

static void test_dev83d_cg_change_runs_pan_when_shut_flag_set_but_pis_count_nonzero(
    test_context *ctx) {
    reset_dev83d_state();
    shut_flag = 1;
    cgchgtim[0] = 0;
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, cgchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 0, cgchgcnt[4]);
    TEST_ASSERT_EQ_INT(ctx, 334, tile_calls[8].tile_start);
    TEST_ASSERT_EQ_INT(ctx, 148, tile_calls[8].bmp_no);
}

static void test_dev83d_cg_change_wait_path_makes_no_tile_calls(
    test_context *ctx) {
    reset_dev83d_state();
    for (int i = 0; i < 5; ++i) {
        cgchgtim[i] = 2;
...
static void test_dev83d_cg_change_emits_exact_tile_sequence(test_context *ctx) {
    reset_dev83d_state();
    cgchgtim[0] = 0;
    cgchgcnt[0] = 0;
    cgchgtim[1] = 0;
    ...
    TEST_ASSERT_EQ_INT(ctx, 40, tile_call_count);
    TEST_ASSERT_EQ_INT(ctx, 6, cgchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 1, cgchgcnt[0]);
    ...
    TEST_ASSERT_EQ_INT(ctx, 7, cgchgtim[3]);
    TEST_ASSERT_EQ_INT(ctx, 6, cgchgcnt[3]);
    TEST_ASSERT_EQ_INT(ctx, 4, cgchgtim[4]);
    TEST_ASSERT_EQ_INT(ctx, 0, cgchgcnt[4]);

    assert_tile_range(ctx, 0, 302, 8, 8);
    assert_tile_range(ctx, 8, 310, 40, 8);
    assert_tile_range(ctx, 16, 318, 104, 16);
    assert_tile_range(ctx, 32, 338, 144, 4);
    assert_tile_range(ctx, 36, 334, 148, 4);
}

TEST_MAIN_BEGIN;
    test_dev83d_map_init_data_captures_literal_values(&ctx);
    test_dev83d_tables_capture_representative_literal_values(&ctx);
    test_dev83d_cg_chg1_waits_while_timer_remains(&ctx);
    test_dev83d_cg_chg1_advances_to_next_frame(&ctx);
    test_dev83d_cg_chg1_wraps_to_first_frame(&ctx);
    test_dev83d_cg_change_skips_pan_when_shut_flag_set_and_pis_count_zero(&ctx);
    test_dev83d_cg_change_runs_pan_when_shut_flag_set_but_pis_count_nonzero(&ctx);
    test_dev83d_cg_change_wait_path_makes_no_tile_calls(&ctx);
    test_dev83d_cg_change_emits_exact_tile_sequence(&ctx);
TEST_MAIN_END
