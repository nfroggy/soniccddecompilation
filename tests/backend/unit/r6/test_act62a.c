...
static void reset_act62a_state(void) {
    frameout_count = 0;
    frameout_actor = 0;
    stub_call_count = 0;
    stub_actor = 0;
...
static void test_act62a_table_matches_current_handlers(test_context *ctx) {
    void (*expected[60])(sprite_status *) = {
        ...
        sw6,    tobira6,  udblk6,   optbr6,   kdai6,     friend,   block6,
        eggz6,  seesaw6,  togeball, et,       movie,     batta,    hachi6,
...
static void test_variant_specific_table_entries_are_callable(test_context *ctx) {
    sprite_status actor;
...
TEST_MAIN_BEGIN;
    test_act62a_table_matches_current_handlers(&ctx);
    test_noact_delegates_to_frameout(&ctx);
    test_variant_specific_table_entries_are_callable(&ctx);
TEST_MAIN_END
