...
static void reset_togebl6a_state(void) {
...
static void reset_togebl6a_logs(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_s_count = 0;
    frameout_s_actor = 0;
...
static void test_togebl6a_tables_capture_literal_data(test_context *ctx) {
...
static void test_togebl6a_main_initializes_and_spawns_options(
    test_context *ctx) {
...
static void test_togebl6a_main_reverses_rotation_from_userflag(
    test_context *ctx) {
    sprite_status *parent = &actwk[2];
...
static void test_togebl6a_spawn_failure_frames_out_each_allocation_point(
    test_context *ctx) {
...
static void test_togebl6a_parent_move_positions_options_through_option_path(
    test_context *ctx) {
    sprite_status *parent = &actwk[2];
    sprite_status *opt0 = &actwk[3];
    sprite_status *opt1 = &actwk[4];
    ...
    parent->xposi.w.h = 100;
    parent->yposi.w.h = 50;
    queue_all_options(opt0, opt1, orphan, opt3, opt4, opt5);
    togeball(parent);
    ...
    reset_togebl6a_logs();
    togeball(opt5);
    TEST_ASSERT_EQ_INT(ctx, 120, opt5->xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 60, opt5->yposi.w.h);
...
static void test_togebl6a_option_initializes_and_moves_existing_position(
    test_context *ctx) {
    sprite_status *parent = &actwk[2];
    ...
    parent->actno = 45;
    opt->userflag.b.h = -1;
    togeball6a_get_work(opt)->parent_index = 2;
    togeball6a_get_work(opt)->target_x = 12;
    togeball6a_get_work(opt)->target_y = 34;
...
static void test_togebl6a_option_frameouts_when_parent_is_gone(
    test_context *ctx) {
    sprite_status *opt = &actwk[3];
...
TEST_MAIN_BEGIN;
    test_togebl6a_tables_capture_literal_data(&ctx);
    test_togebl6a_main_initializes_and_spawns_options(&ctx);
    test_togebl6a_main_reverses_rotation_from_userflag(&ctx);
    test_togebl6a_spawn_failure_frames_out_each_allocation_point(&ctx);
    test_togebl6a_parent_move_positions_options_through_option_path(&ctx);
    test_togebl6a_option_initializes_and_moves_existing_position(&ctx);
    test_togebl6a_option_frameouts_when_parent_is_gone(&ctx);
TEST_MAIN_END
