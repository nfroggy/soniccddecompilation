...
#include "src/r6/dev63c.c"

static void reset_dev63c_state(void) {
    bossflag = 0;
...
static void assert_sp_tail(test_context *ctx, int offset) {
    int i;

    for (i = 0; i < 4; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 343 + i, tile_calls[offset + i].tile_start);
        TEST_ASSERT_EQ_INT(ctx, 5 + i, tile_calls[offset + i].bmp_no);
    }
}

static void test_cg_change_emits_thunder_and_sp(test_context *ctx) {
    int i;
    ...
    for (i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 347 + i, tile_calls[i].tile_start);
        TEST_ASSERT_EQ_INT(ctx, 17 + i, tile_calls[i].bmp_no);
    }
    assert_sp_tail(ctx, 8);
}
...
test_cg_change_emits_thunder_and_sp(&ctx);
