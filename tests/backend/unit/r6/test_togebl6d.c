...
static void reset_togebl6d_state(void) {
...
static void reset_togebl6d_logs(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_s_count = 0;
    frameout_s_actor = 0;
...
static void test_togebl6d_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, SPRITE_TOGEBL6D_BASE, spat_chg0.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, -8, spat_chg1.spra[0].xoff);
    TEST_ASSERT_EQ_INT(ctx, -8, spat_chg1.spra[0].yoff);
    TEST_ASSERT_EQ_INT(ctx, SPRITE_TOGEBL6D_BASE + 1, spat_chg1.spra[0].index);
}

static void test_togebl6d_main_initializes_and_spawns_options(
    test_context *ctx) {
    ...
    sprite_status *opt5 = &actwk[8];

    reset_togebl6d_state();
    parent->actno = 45;
    parent->xposi.w.h = 100;
    parent->yposi.w.h = 50;
    queue_all_options(opt0, opt1, opt2, opt3, opt4, opt5);
    ...
    TEST_ASSERT_EQ_INT(ctx, 45, opt5->actno);
    TEST_ASSERT_EQ_INT(ctx, -1, opt5->userflag.b.h);
    assert_main_callbacks(ctx, parent);
}

static void test_togebl6d_main_reverses_rotation_from_userflag(
    test_context *ctx) {
    sprite_status *parent = &actwk[2];
...
static void test_togebl6d_spawn_failure_frames_out_each_allocation_point(
    test_context *ctx) {
...
static void test_togebl6d_parent_move_positions_all_options(test_context *ctx) {
    sprite_status *parent = &actwk[2];
    sprite_status *opt0 = &actwk[3];
    sprite_status *opt1 = &actwk[4];
    sprite_status *opt2 = &actwk[5];
    ...
    reset_togebl6d_logs();
    togeball(opt2);
    TEST_ASSERT_EQ_INT(ctx, 112, opt2->xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 56, opt2->yposi.w.h);

    reset_togebl6d_logs();
    togeball(opt3);
    TEST_ASSERT_EQ_INT(ctx, 116, opt3->xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 58, opt3->yposi.w.h);

    reset_togebl6d_logs();
    togeball(opt4);
    TEST_ASSERT_EQ_INT(ctx, 120, opt4->xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 60, opt4->yposi.w.h);

    reset_togebl6d_logs();
    togeball(opt5);
    TEST_ASSERT_EQ_INT(ctx, 124, opt5->xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 62, opt5->yposi.w.h);
}

static void test_togebl6d_option_initializes_and_moves_existing_position(
    test_context *ctx) {
    sprite_status *parent = &actwk[2];
...
static void test_togebl6d_option_frameouts_when_parent_is_gone(
    test_context *ctx) {
    sprite_status *opt = &actwk[3];
...
TEST_MAIN_BEGIN;
    test_togebl6d_tables_capture_literal_data(&ctx);
    test_togebl6d_main_initializes_and_spawns_options(&ctx);
    test_togebl6d_main_reverses_rotation_from_userflag(&ctx);
    test_togebl6d_spawn_failure_frames_out_each_allocation_point(&ctx);
    test_togebl6d_parent_move_positions_all_options(&ctx);
    test_togebl6d_option_initializes_and_moves_existing_position(&ctx);
    test_togebl6d_option_frameouts_when_parent_is_gone(&ctx);
TEST_MAIN_END
