...
#include "src/r6/col6c.c"

static void reset_col6c_state(void) {
...
static void test_col6c_tables_capture_palette_contract(test_context *ctx) {
    ...
    assert_palette(ctx, advacolor[63], 0, 128, 224, 1, "advacolor[63]");
    assert_palette(ctx, advacolor2[33], 224, 224, 0, 1, "advacolor2[33]");
    assert_palette(ctx, gamecolor[15], 224, 224, 0, 1, "gamecolor[15]");
    assert_palette(ctx, zone8colora[47], 0, 128, 224, 1, "zone8colora[47]");
    assert_palette(ctx, zone82colora[47], 0, 128, 224, 1,
                   "zone82colora[47]");
    assert_palette(ctx, boss6col[15], 224, 0, 0, 1, "boss6col[15]");
    assert_palette(ctx, boss6_col4[79], 224, 224, 224, 1, "boss6_col4[79]");
    ...
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[6].cnt);

    TEST_ASSERT_EQ_INT(ctx, 33, col6a0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 49, col6a3cnt[0]);
    assert_palette(ctx, col6a0col[1], 0, 192, 0, 1, "col6a0col[1]");
    assert_palette(ctx, col6a3col[1], 64, 0, 224, 1, "col6a3col[1]");
...
static void test_colchg6a_updates_running_and_stopped_boss_cycles(
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgtim[1]);
    TEST_ASSERT_EQ_INT(ctx, 1, clchgtim[2]);
    TEST_ASSERT_EQ_INT(ctx, 1, clchgtim[3]);
    assert_palette(ctx, colorwk[33], 0, 192, 0, 1, "cycle color 33");
    assert_palette(ctx, colorwk[34], 0, 64, 0, 1, "cycle color 34");
    assert_palette(ctx, colorwk[35], 224, 224, 0, 1, "cycle color 35");
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 0, 128, 224, 1, "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "colorset3 work 0");
    assert_palette(ctx, colorwk[63], 0, 128, 224, 1, "colorset3 work 0 end");

    colorset00(0, 2);
    assert_palette(ctx, colorwk3[0], 0, 0, 0, 1, "colorset00 work 2");
    assert_palette(ctx, colorwk3[63], 0, 128, 224, 1,
                   "colorset00 work 2 end");

    colorset00(0, 3);
    assert_palette(ctx, colorwk4[0], 0, 0, 0, 1, "colorset00 default");
    assert_palette(ctx, colorwk4[63], 0, 128, 224, 1,
                   "colorset00 default end");

    colorset(6);
    assert_palette(ctx, colorwk2[16], 0, 0, 0, 1, "colorset boss");
    assert_palette(ctx, colorwk2[31], 224, 0, 0, 1, "colorset boss end");
}

static void test_boss_fade_helpers_step_and_clamp(test_context *ctx) {
    Uint8 chg_time;
    Uint8 chg_cnt;
    ...
    assert_palette(ctx, colorwk[55], 0, 64, 32, 1, "fin boss 55");
    assert_palette(ctx, colorwk[63], 0, 128, 224, 1, "fin boss 63");
    ...
    TEST_ASSERT_EQ_INT(ctx, 11, chg_time);
    TEST_ASSERT_EQ_INT(ctx, 30, chg_cnt);
    ...
    assert_palette(ctx, colorwk[63], 96, 224, 224, 1, "fout boss 63");
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, chg_time);
    TEST_ASSERT_EQ_INT(ctx, 70, chg_cnt);
    assert_palette(ctx, colorwk[32], 224, 224, 224, 1,
                   "fout boss clamped first");

    fset_boss6(20, &chg_cnt);
    TEST_ASSERT_EQ_INT(ctx, 20, chg_cnt);
    assert_palette(ctx, colorwk[32], 64, 64, 64, 1, "fset boss first");
    assert_palette(ctx, colorwk[55], 64, 128, 96, 1, "fset boss 55");
    assert_palette(ctx, colorwk[63], 64, 192, 224, 1, "fset boss 63");
}

static void test_flashin_boss6_initializes_shorter_range(test_context *ctx) {
    reset_col6c_state();
...
TEST_MAIN_BEGIN;
test_col6c_tables_capture_palette_contract(&ctx);
...
test_boss_fade_helpers_step_and_clamp(&ctx);
test_flashin_boss6_initializes_shorter_range(&ctx);
