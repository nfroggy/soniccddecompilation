...
static void reset_et6_state(void) {
...
static void reset_et6_logs(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_s_count = 0;
    frameout_s_actor = 0;
...
static void test_et6_tables_capture_literal_data(test_context *ctx) {
    TEST_ASSERT_TRUE(ctx, pat_et[0] == &tpat00);
    TEST_ASSERT_TRUE(ctx, pat_et[7] == &tpat07);
    TEST_ASSERT_EQ_INT(ctx, -40, tpat00.spra[0].xoff);
    TEST_ASSERT_EQ_INT(ctx, 419, tpat00.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, 4, tpat06.cnt);
    TEST_ASSERT_EQ_INT(ctx, 434, tpat06.spra[1].index);
    TEST_ASSERT_EQ_INT(ctx, 427, tpat06.spra[3].index);
...
static void test_et6_entry_initializes_default_and_waits(test_context *ctx) {
...
static void test_et6_init_variants_capture_pattern_and_wait_gates(
    test_context *ctx) {
    sprite_status *actor = &actwk[4];
...
static void test_et6_entry_dispatches_die_and_reset_states(test_context *ctx) {
    sprite_status *actor = &actwk[4];
...
TEST_MAIN_BEGIN;
    test_et6_tables_capture_literal_data(&ctx);
    test_et6_entry_initializes_default_and_waits(&ctx);
    test_et6_init_variants_capture_pattern_and_wait_gates(&ctx);
    ...
    test_et6_entry_dispatches_die_and_reset_states(&ctx);
