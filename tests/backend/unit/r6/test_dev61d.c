...
#include "src/r6/dev61d.c"

static void reset_dev61d_state(void) {
...
static void test_cg_change_emits_exact_tile_sequence(test_context *ctx) {
    ...
    for (i = 0; i < 4; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 376 + i, tile_calls[8 + i].tile_start);
        TEST_ASSERT_EQ_INT(ctx, 4 + i, tile_calls[8 + i].bmp_no);
    }
    for (i = 0; i < 4; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 372 + i, tile_calls[12 + i].tile_start);
