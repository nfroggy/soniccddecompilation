...
#include "src/r6/col6a.c"

static void reset_col6a_state(void) {
...
static void test_col6a_tables_capture_palette_contract(test_context *ctx) {
    ...
    assert_palette(ctx, zone8colora[47], 0, 224, 224, 1, "zone8colora[47]");
    assert_palette(ctx, zone82colora[47], 0, 224, 224, 1,
                   "zone82colora[47]");
    ...
    assert_palette(ctx, col6a0col[1], 224, 0, 0, 1, "col6a0col[1]");
    assert_palette(ctx, col6a3col[1], 128, 32, 192, 1, "col6a3col[1]");
}
...
static void test_colchg6a_updates_all_cycles(test_context *ctx) {
    reset_col6a_state();
    ...
    assert_palette(ctx, colorwk[33], 224, 0, 0, 1, "cycle color 33");
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 0, 224, 224, 1, "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "colorset3 work 0");
    assert_palette(ctx, colorwk[63], 0, 224, 224, 1, "colorset3 work 0 end");
...
TEST_MAIN_BEGIN;
test_col6a_tables_capture_palette_contract(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg6a_updates_all_cycles(&ctx);
