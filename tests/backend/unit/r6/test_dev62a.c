...
#include "src/r6/dev62a.c"

static void reset_dev62a_state(void) {
...
static void test_tables_capture_tile_animation_contract(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 321, tbl_thund.TileStart);
    TEST_ASSERT_EQ_INT(ctx, 2, tbl_sp.chgcnt);
    TEST_ASSERT_EQ_INT(ctx, 317, tbl_sp.TileStart);
    TEST_ASSERT_EQ_INT(ctx, 4, tbl_light.chgcnt);
    TEST_ASSERT_EQ_INT(ctx, 313, tbl_light.TileStart);
...
static void test_cg_chg1_waits_advances_and_wraps(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, cgchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 321, tile_start);
    for (i = 0; i < 8; ++i) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 317, tile_start);
    for (i = 0; i < 4; ++i) {
...
static void test_cg_change_emits_exact_tile_sequence(test_context *ctx) {
    ...
    for (i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 321 + i, tile_calls[i].tile_start);
        TEST_ASSERT_EQ_INT(ctx, 16 + i, tile_calls[i].bmp_no);
    }
    for (i = 0; i < 4; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 317 + i, tile_calls[8 + i].tile_start);
        TEST_ASSERT_EQ_INT(ctx, 4 + i, tile_calls[8 + i].bmp_no);
    }
    for (i = 0; i < 4; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 313 + i, tile_calls[12 + i].tile_start);
