...
#include "src/r6/col6b.c"

static void reset_col6b_state(void) {
...
static void test_col6b_tables_capture_palette_contract(test_context *ctx) {
    ...
    assert_palette(ctx, zone3colora[0], 96, 96, 96, 1, "zone3colora[0]");
    assert_palette(ctx, zone3colora[47], 0, 224, 0, 1, "zone3colora[47]");
    assert_palette(ctx, advacolor[0], 0, 32, 160, 1, "advacolor[0]");
    assert_palette(ctx, advacolor[63], 224, 192, 0, 1, "advacolor[63]");
    assert_palette(ctx, advacolor2[33], 224, 0, 224, 1, "advacolor2[33]");
    assert_palette(ctx, gamecolor[15], 224, 224, 0, 1, "gamecolor[15]");
    assert_palette(ctx, zone8colora[47], 224, 0, 32, 1, "zone8colora[47]");
    assert_palette(ctx, zone82colora[47], 224, 0, 32, 1,
                   "zone82colora[47]");
    ...
    assert_palette(ctx, col6a0col[1], 96, 0, 96, 1, "col6a0col[1]");
    assert_palette(ctx, col6a3col[1], 32, 64, 128, 1, "col6a3col[1]");
}
...
static void test_colchg6b_updates_all_cycles(test_context *ctx) {
    reset_col6b_state();
    ...
    assert_palette(ctx, colorwk[33], 96, 0, 96, 1, "cycle color 33");
    assert_palette(ctx, colorwk[34], 0, 0, 0, 1, "cycle color 34");
    assert_palette(ctx, colorwk[35], 224, 0, 224, 1, "cycle color 35");
    assert_palette(ctx, colorwk[49], 0, 0, 0, 1, "cycle color 49");
}
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 224, 0, 32, 1, "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "colorset3 work 0");
    assert_palette(ctx, colorwk[63], 224, 0, 32, 1, "colorset3 work 0 end");
...
TEST_MAIN_BEGIN;
test_col6b_tables_capture_palette_contract(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg6b_updates_all_cycles(&ctx);
