...
#define EDIT_DATA_ENTRY_COUNT 23
#include "src/types.h"
...
static void test_edtbl31d_entries_match_existing_table(test_context *ctx) {
    TEST_ASSERT_EQ_INT(ctx, 23, edit_tbl.num);
    ...
    assert_edit_entry(ctx, 2, 45, 1, pat_e_ga, 9249, 0, 0, 0, 0);
    assert_edit_entry(ctx, 3, 45, 1, pat_b_ga, 9249, 255, 0, 0, 0);
    assert_edit_entry(ctx, 4, 46, 1, pat_e_tentou, 9322, 0, 0, 0, 0);
    assert_edit_entry(ctx, 5, 46, 1, pat_ten_b_tentou, 9322, 255, 0, 0,
                      0);
    assert_edit_entry(ctx, 6, 36, 1, kamapat, 9202, 0, 0, 0, 0);
    assert_edit_entry(ctx, 7, 36, 1, bkamapat, 9202, 255, 0, 0, 0);
    assert_edit_entry(ctx, 8, 32, 1, harir3pat, 798, 0, 0, 0, 0);
    assert_edit_entry(ctx, 9, 33, 1, frdr3pat, 17214, 2, 0, 0, 0);
    assert_edit_entry(ctx, 10, 34, 1, trapdr3pat, 846, 1, 0, 0, 0);
    assert_edit_entry(ctx, 11, 35, 1, for3pat, 862, 5, 0, 0, 0);
    assert_edit_entry(ctx, 12, 38, 1, getdair3pat, 878, 0, 0, 0, 0);
    assert_edit_entry(ctx, 13, 39, 1, gandair3pat, 886, 0, 0, 0, 0);
    assert_edit_entry(ctx, 14, 40, 1, drumr3pat, 910, 0, 0, 0, 0);
    assert_edit_entry(ctx, 15, 47, 1, pat_pocket, 957, 0, 0, 0, 0);
    assert_edit_entry(ctx, 16, 42, 1, banpa_pat, 17246, 0, 0, 0, 0);
    assert_edit_entry(ctx, 17, 44, 1, togeball_pat, 957, 0, 0, 0, 0);
    assert_edit_entry(ctx, 18, 29, 1, bobinpat, 1230, 0, 0, 0, 0);
    assert_edit_entry(ctx, 19, 30, 1, frippat, 1181, 0, 0, 0, 0);
    assert_edit_entry(ctx, 20, 30, 1, frippat, 1181, 0, 1, 0, 0);
    assert_edit_entry(ctx, 21, 10, 1, springpat, 1312, 0, 0, 0, 0);
    assert_edit_entry(ctx, 22, 25, 4, itempat, 1448, 0, 0, 0, 0);
}

TEST_MAIN_BEGIN;
    test_edtbl31d_entries_match_existing_table(&ctx);
TEST_MAIN_END
