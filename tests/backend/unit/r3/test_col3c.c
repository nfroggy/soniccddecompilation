...
#include "src/r3/col3c.c"

static void reset_col3c_state(void) {
...
static void test_col3c_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone3colorc12) /
                             sizeof(zone3colorc12[0])));
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone3colorc) / sizeof(zone3colorc[0])));
    TEST_ASSERT_EQ_INT(ctx, 16,
    ...
    TEST_ASSERT_EQ_INT(ctx, 8, (int)(sizeof(colortbl) / sizeof(colortbl[0])));

    assert_palette(ctx, advacolor[0], 0, 32, 160, 1, "advacolor[0]");
    assert_palette(ctx, advacolor2[49], 192, 224, 224, 1,
                   "advacolor2[49]");
    assert_palette(ctx, gamecolor[0], 32, 32, 160, 1, "gamecolor[0]");
    assert_palette(ctx, zone3colorc12[47], 0, 0, 0, 1,
                   "zone3colorc12[47]");
    assert_palette(ctx, zone3colorc[47], 96, 128, 64, 1,
                   "zone3colorc[47]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone3colorc);
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == zone3colboss0);
    TEST_ASSERT_TRUE(ctx, colortbl[6].PeSrc == zone3colboss1);
    TEST_ASSERT_TRUE(ctx, colortbl[7].PeSrc == zone3colorc12);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 48, colortbl[7].cnt);

    TEST_ASSERT_EQ_INT(ctx, 44, col3c0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 3, col3c0cnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 61, col3c1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 16, col3c1cnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 60, col3c1cnt[16]);
    assert_palette(ctx, col3c0col[1], 224, 0, 224, 1, "col3c0col[1]");
    assert_palette(ctx, col3c1col[0], 224, 160, 0, 1, "col3c1col[0]");
}
...
static void test_colchg3c_updates_two_cycle_slots(test_context *ctx) {
    reset_col3c_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, clchgtim[2]);
    assert_palette(ctx, colorwk[44], 224, 0, 224, 1, "cycle color 44");
    assert_palette(ctx, colorwk[61], 0, 0, 0, 1, "cycle color 61");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    colorset(3);
    assert_palette(ctx, colorwk2[0], 32, 32, 160, 1, "colorset work 1");
    assert_palette(ctx, colorwk2[15], 224, 224, 0, 1, "colorset work 1 end");

    colorset2(4);
    assert_palette(ctx, colorwk[16], 96, 96, 96, 1, "colorset2 work 0");
    assert_palette(ctx, colorwk[63], 96, 128, 64, 1,
                   "colorset2 work 0 end");

    colorset3(7);
    assert_palette(ctx, colorwk[63], 0, 0, 0, 1, "colorset3 c12 end");
...
TEST_MAIN_BEGIN;
test_col3c_tables_capture_literal_palette_data(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg3c_updates_two_cycle_slots(&ctx);
