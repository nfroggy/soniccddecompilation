...
DECLARE_ACT_STUB(egg3dai_s);
DECLARE_ACT_STUB(egg3dai_l);
DECLARE_ACT_STUB(egg3haguruma);
DECLARE_ACT_STUB(egg3);
DECLARE_ACT_STUB(ball);
DECLARE_ACT_STUB(sikake33);
DECLARE_ACT_STUB(et);
...
DECLARE_ACT_STUB(egg3cannon);
DECLARE_ACT_STUB(punchbom);
DECLARE_ACT_STUB(gate);
#undef DECLARE_ACT_STUB

#include "src/r3/act33c.c"
...
DEFINE_ACT_STUB(egg3dai_s)
DEFINE_ACT_STUB(egg3dai_l)
DEFINE_ACT_STUB(egg3haguruma)
DEFINE_ACT_STUB(egg3)
DEFINE_ACT_STUB(ball)
DEFINE_ACT_STUB(sikake33)
DEFINE_ACT_STUB(et)
...
DEFINE_ACT_STUB(egg3cannon)
DEFINE_ACT_STUB(punchbom)
DEFINE_ACT_STUB(gate)
#undef DEFINE_ACT_STUB

static void reset_act33c_state(void) {
    frameout_count = 0;
    frameout_actor = 0;
    stub_call_count = 0;
    stub_actor = 0;
...
static void test_act33c_table_captures_all_dispatch_slots(test_context *ctx) {
    TEST_ASSERT_EQ_INT(ctx, 63, (int)(sizeof(act_tbl) / sizeof(act_tbl[0])));
    ...
    assert_act_entry(ctx, 47, egg3dai_s);
    assert_act_entry(ctx, 48, egg3dai_l);
    assert_act_entry(ctx, 49, egg3haguruma);
    assert_act_entry(ctx, 50, egg3);
    assert_act_entry(ctx, 51, noact);
    assert_act_entry(ctx, 52, ball);
    assert_act_entry(ctx, 53, sikake33);
    ...
    assert_act_entry(ctx, 60, egg3cannon);
    assert_act_entry(ctx, 61, punchbom);
    assert_act_entry(ctx, 62, gate);
}

static void test_act33c_table_entries_dispatch_to_selected_handlers(
    test_context *ctx) {
    sprite_status actor;
...
static void test_noact_frameouts_actor(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 12, actor.actno);
    TEST_ASSERT_EQ_INT(ctx, 777, actor.xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 888, actor.yposi.w.h);
}

TEST_MAIN_BEGIN;
    test_act33c_table_captures_all_dispatch_slots(&ctx);
    test_act33c_table_entries_dispatch_to_selected_handlers(&ctx);
