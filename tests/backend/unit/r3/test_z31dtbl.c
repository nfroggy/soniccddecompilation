...
static void test_z31dtbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 1060764671u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 2022794527u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 793237129u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z31dtbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z31dtbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, zone1scd[651]);
    TEST_ASSERT_EQ_INT(ctx, 0, zone1scd[999]);

    TEST_ASSERT_EQ_INT(ctx, 21, mapwka[0][10]);
    TEST_ASSERT_EQ_INT(ctx, 95, mapwka[0][14]);
    TEST_ASSERT_EQ_INT(ctx, 95, mapwka[1][16]);
...
TEST_MAIN_BEGIN;
test_z31dtbl_table_sizes_and_fingerprints(&ctx);
test_z31dtbl_direction_and_collision_anchors(&ctx);
test_z31dtbl_map_anchors(&ctx);
TEST_MAIN_END
