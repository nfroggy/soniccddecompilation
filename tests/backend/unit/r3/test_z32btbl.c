...
static void test_z32btbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 1133667664u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 2363726198u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 2709233862u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z32btbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z32btbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 96, mapwka[1][0]);
    TEST_ASSERT_EQ_INT(ctx, 84, mapwka[2][27]);
    TEST_ASSERT_EQ_INT(ctx, 99, mapwka[4][5]);
    TEST_ASSERT_EQ_INT(ctx, 54, mapwka[5][15]);
    TEST_ASSERT_EQ_INT(ctx, 94, mapwka[5][27]);
...
TEST_MAIN_BEGIN;
test_z32btbl_table_sizes_and_fingerprints(&ctx);
test_z32btbl_direction_and_collision_anchors(&ctx);
test_z32btbl_map_anchors(&ctx);
TEST_MAIN_END
