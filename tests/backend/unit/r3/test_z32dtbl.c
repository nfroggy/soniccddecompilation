...
static void test_z32dtbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 1338966181u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 2761258177u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 793237129u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z32dtbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z32dtbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[0][1]);
    TEST_ASSERT_EQ_INT(ctx, 95, mapwka[0][3]);
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[0][19]);
    TEST_ASSERT_EQ_INT(ctx, 95, mapwka[1][18]);
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[1][32]);
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[2][8]);
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[2][16]);
    TEST_ASSERT_EQ_INT(ctx, 95, mapwka[3][30]);
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[4][32]);
    TEST_ASSERT_EQ_INT(ctx, 86, mapwka[5][38]);
...
TEST_MAIN_BEGIN;
test_z32dtbl_table_sizes_and_fingerprints(&ctx);
test_z32dtbl_direction_and_collision_anchors(&ctx);
test_z32dtbl_map_anchors(&ctx);
TEST_MAIN_END
