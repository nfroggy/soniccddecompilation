...
static void reset_act31b_state(void) {
    frameout_count = 0;
    frameout_actor = 0;
    stub_call_count = 0;
    stub_actor = 0;
...
static void test_act31b_table_captures_all_dispatch_slots(test_context *ctx) {
...
static void test_act31b_table_entries_dispatch_to_selected_handlers(
    test_context *ctx) {
    sprite_status actor;
...
static void test_noact_frameouts_actor(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 11, actor.actno);
    TEST_ASSERT_EQ_INT(ctx, 555, actor.xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 666, actor.yposi.w.h);
}

TEST_MAIN_BEGIN;
    test_act31b_table_captures_all_dispatch_slots(&ctx);
    test_act31b_table_entries_dispatch_to_selected_handlers(&ctx);
