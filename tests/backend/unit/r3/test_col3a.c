...
#include "src/r3/col3a.c"

static void reset_col3a_state(void) {
...
static void test_col3a_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone3colora) / sizeof(zone3colora[0])));
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone3colora2) / sizeof(zone3colora2[0])));
    ...
    assert_palette(ctx, gamecolor[0], 128, 0, 128, 1, "gamecolor[0]");
    assert_palette(ctx, zone3colora[23], 32, 224, 0, 1, "zone3colora[23]");
    assert_palette(ctx, zone3colora2[23], 0, 224, 0, 1,
                   "zone3colora2[23]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone3colora);
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == zone3colora2);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 48, colortbl[5].cnt);

    TEST_ASSERT_EQ_INT(ctx, 47, col3a0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 2, col3a0cnt[6]);
    TEST_ASSERT_EQ_INT(ctx, 60, col3a1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 8, col3a1cnt[32]);
    TEST_ASSERT_EQ_INT(ctx, 61, col3a2cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 22, col3a2cnt[2]);
    TEST_ASSERT_EQ_INT(ctx, 63, col3a3cnt[0]);
    assert_palette(ctx, col3a0col[0], 224, 224, 0, 1, "col3a0col[0]");
    assert_palette(ctx, col3a2col[2], 224, 160, 224, 1, "col3a2col[2]");
}

static void test_cntplus_wraps_only_above_upper_limit(test_context *ctx) {
    Uint8 work;

    work = 5;
    TEST_ASSERT_EQ_INT(ctx, 8, cntplus(&work, 3, 2, 10));
    TEST_ASSERT_EQ_INT(ctx, 8, work);

    work = 9;
    TEST_ASSERT_EQ_INT(ctx, 2, cntplus(&work, 3, 2, 10));
    TEST_ASSERT_EQ_INT(ctx, 2, work);
}
...
static void test_colchg3a_updates_all_four_cycle_slots(test_context *ctx) {
    reset_col3a_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 2, clchgtim[1]);
    TEST_ASSERT_EQ_INT(ctx, 2, clchgtim[2]);
    TEST_ASSERT_EQ_INT(ctx, 2, clchgtim[3]);
    assert_palette(ctx, colorwk[47], 224, 0, 0, 1, "cycle color 47");
    assert_palette(ctx, colorwk[60], 0, 0, 0, 1, "cycle color 60");
    assert_palette(ctx, colorwk[61], 224, 0, 224, 1, "cycle color 61");
    assert_palette(ctx, colorwk[63], 0, 0, 0, 1, "cycle color 63");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    colorset(3);
    assert_palette(ctx, colorwk2[0], 128, 0, 128, 1, "colorset work 1");
    ...
    colorset3(5);
    assert_palette(ctx, colorwk[39], 0, 224, 0, 1, "colorset3 work 0");
...
TEST_MAIN_BEGIN;
test_col3a_tables_capture_literal_palette_data(&ctx);
test_cntplus_wraps_only_above_upper_limit(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg3a_updates_all_four_cycle_slots(&ctx);
