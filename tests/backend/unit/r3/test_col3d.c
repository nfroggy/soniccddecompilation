...
#include "src/r3/col3d.c"

static void reset_col3d_state(void) {
...
static void test_col3d_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone3colord) / sizeof(zone3colord[0])));
    TEST_ASSERT_EQ_INT(ctx, 16,
                       (int)(sizeof(zone3colboss0) /
                             sizeof(zone3colboss0[0])));
    TEST_ASSERT_EQ_INT(ctx, 16,
                       (int)(sizeof(zone3colboss1) /
                             sizeof(zone3colboss1[0])));
    TEST_ASSERT_EQ_INT(ctx, 7, (int)(sizeof(colortbl) / sizeof(colortbl[0])));

    assert_palette(ctx, advacolor[0], 0, 32, 160, 1, "advacolor[0]");
    assert_palette(ctx, advacolor2[49], 192, 224, 224, 1,
                   "advacolor2[49]");
    assert_palette(ctx, gamecolor[0], 64, 32, 32, 1, "gamecolor[0]");
    assert_palette(ctx, zone3colord[16], 32, 32, 0, 1,
                   "zone3colord[16]");
    assert_palette(ctx, zone3colord[47], 224, 224, 32, 1,
                   "zone3colord[47]");
    assert_palette(ctx, zone3colboss0[2], 224, 224, 224, 1,
                   "zone3colboss0[2]");
    assert_palette(ctx, zone3colboss1[10], 128, 96, 160, 1,
                   "zone3colboss1[10]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone3colord);
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == zone3colboss0);
    TEST_ASSERT_TRUE(ctx, colortbl[6].PeSrc == zone3colboss1);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 48, colortbl[4].cnt);

    TEST_ASSERT_EQ_INT(ctx, 8, (int)(sizeof(col3d0cnt) / sizeof(col3d0cnt[0])));
    TEST_ASSERT_EQ_INT(ctx, 36,
                       (int)(sizeof(col3d1cnt) / sizeof(col3d1cnt[0])));
    TEST_ASSERT_EQ_INT(ctx, 34,
                       (int)(sizeof(col3d2cnt) / sizeof(col3d2cnt[0])));
    TEST_ASSERT_EQ_INT(ctx, 42, col3d0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 3, col3d0cnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 59, col3d1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 17, col3d1cnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 61, col3d2cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 16, col3d2cnt[1]);
    assert_palette(ctx, col3d0col[1], 0, 192, 96, 1, "col3d0col[1]");
    assert_palette(ctx, col3d1col[8], 224, 224, 128, 1, "col3d1col[8]");
    assert_palette(ctx, col3d2col[8], 0, 0, 0, 1, "col3d2col[8]");
}
...
static void test_colchg3d_updates_three_cycle_slots(test_context *ctx) {
    reset_col3d_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 9, clchgtim[1]);
    TEST_ASSERT_EQ_INT(ctx, 9, clchgtim[2]);
    assert_palette(ctx, colorwk[42], 0, 192, 96, 1, "cycle color 42");
    assert_palette(ctx, colorwk[59], 128, 0, 224, 1, "cycle color 59");
    assert_palette(ctx, colorwk[61], 224, 160, 0, 1, "cycle color 61");
}
...
static void test_fadein_initializes_range_and_steps_toward_sources(
    ...
    colorwk[3] = (PALETTEENTRY){99, 88, 77, 0};
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    colorset(3);
    assert_palette(ctx, colorwk2[0], 64, 32, 32, 1, "colorset work 1");
    assert_palette(ctx, colorwk2[15], 224, 224, 0, 1, "colorset work 1 end");

    colorset2(4);
    assert_palette(ctx, colorwk[16], 96, 96, 96, 1, "colorset2 work 0");
    assert_palette(ctx, colorwk[63], 224, 224, 32, 1,
                   "colorset2 work 0 end");

    colorset3(4);
    assert_palette(ctx, colorwk[58], 0, 64, 0, 1, "colorset3 work 0");

    colorset00(5, 0);
    assert_palette(ctx, colorwk[18], 224, 224, 224, 1, "colorset00 boss0");

    colorset00(6, 2);
    assert_palette(ctx, colorwk3[26], 128, 96, 160, 1, "colorset00 boss1");
...
TEST_MAIN_BEGIN;
test_col3d_tables_capture_literal_palette_data(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg3d_updates_three_cycle_slots(&ctx);
