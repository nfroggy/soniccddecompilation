...
static void test_togebl3a_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, SPRITE_TOGEBL3A_BASE, spat_chg0.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, -8, spat_chg1.spra[0].xoff);
    TEST_ASSERT_EQ_INT(ctx, -8, spat_chg1.spra[0].yoff);
    TEST_ASSERT_EQ_INT(ctx, SPRITE_TOGEBL3A_BASE + 1, spat_chg1.spra[0].index);
}
...
static void test_togebl3a_option_initializes_and_moves_existing_position(
    ...
    parent->actno = 44;
    opt->userflag.b.h = -1;
    togeball3a_get_work(opt)->parent_index = 2;
    togeball3a_get_work(opt)->target_x = 12 << 16;
    togeball3a_get_work(opt)->target_y = 34 << 16;
