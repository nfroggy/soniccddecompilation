...
#include "src/r3/dev32c.c"

static void reset_dev32c_state(void) {
...
static void assert_map_init_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 7, mapinittbl.colorno1);
    TEST_ASSERT_EQ_INT(ctx, 7, mapinittbl.colorno2);
...
static void test_cg_chg1_waits_while_timer_remains(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 0,
                       cg_chg1(&tbl_drumg00, 9, &tim, &cnt, bmp_no,
                               &tile_start));
...
static void test_cg_chg1_wraps_to_first_frame(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 1,
                       cg_chg1(&tbl_drumg01, 7, &tim, &cnt, bmp_no,
                               &tile_start));
    TEST_ASSERT_EQ_INT(ctx, 10, cgchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 0, cgchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 464, tile_start);
    for (i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 28 + i, bmp_no[i]);
    }
}
...
static void test_cg_change_emits_exact_tile_sequence(test_context *ctx) {
    ...
    for (i = 0; i < 10; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 438 + i, tile_calls[i].tile_start);
        TEST_ASSERT_EQ_INT(ctx, i, tile_calls[i].bmp_no);
    }
    for (i = 0; i < 8; ++i) {
        TEST_ASSERT_EQ_INT(ctx, 464 + i, tile_calls[10 + i].tile_start);
