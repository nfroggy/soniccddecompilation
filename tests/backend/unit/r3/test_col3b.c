...
#include "src/r3/col3b.c"

static void reset_col3b_state(void) {
...
static void test_col3b_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone3colorb) / sizeof(zone3colorb[0])));
    ...
    assert_palette(ctx, gamecolor[0], 192, 96, 32, 1, "gamecolor[0]");
    assert_palette(ctx, zone3colorb[16], 192, 96, 32, 1,
                   "zone3colorb[16]");
    assert_palette(ctx, zone3colorb[23], 0, 224, 64, 1,
                   "zone3colorb[23]");
    assert_palette(ctx, zone3colorb[47], 224, 224, 224, 1,
                   "zone3colorb[47]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone3colorb);
...
static void test_clchgctr_is_empty_for_col3b(test_context *ctx) {
    reset_col3b_state();
    clchgtim[0] = 7;
    clchgcnt[0] = 3;
    colorwk[12] = (PALETTEENTRY){1, 2, 3, 4};

    clchgctr();

    TEST_ASSERT_EQ_INT(ctx, 7, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 3, clchgcnt[0]);
    assert_palette(ctx, colorwk[12], 1, 2, 3, 4, "empty clchgctr color");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    colorset(3);
    assert_palette(ctx, colorwk2[0], 192, 96, 32, 1, "colorset work 1");
    ...
    colorset3(4);
    assert_palette(ctx, colorwk[39], 0, 224, 64, 1, "colorset3 work 0");
...
TEST_MAIN_BEGIN;
test_col3b_tables_capture_literal_palette_data(&ctx);
test_clchgctr_is_empty_for_col3b(&ctx);
