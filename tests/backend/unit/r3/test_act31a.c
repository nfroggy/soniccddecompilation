...
DECLARE_ACT_STUB(hari3x);
DECLARE_ACT_STUB(mecasnc3);
DECLARE_ACT_STUB(emie3);
DECLARE_ACT_STUB(heart3);
DECLARE_ACT_STUB(msnc3fire);
...
DEFINE_ACT_STUB(hari3x)
DEFINE_ACT_STUB(mecasnc3)
DEFINE_ACT_STUB(emie3)
DEFINE_ACT_STUB(heart3)
DEFINE_ACT_STUB(msnc3fire)
...
static void reset_act31a_state(void) {
    frameout_count = 0;
    frameout_actor = 0;
    stub_call_count = 0;
    stub_actor = 0;
...
static void test_act31a_table_captures_all_dispatch_slots(test_context *ctx) {
    ...
    assert_act_entry(ctx, 47, hari3x);
    assert_act_entry(ctx, 48, mecasnc3);
    assert_act_entry(ctx, 49, emie3);
    assert_act_entry(ctx, 50, heart3);
    assert_act_entry(ctx, 51, msnc3fire);
...
static void test_act31a_table_entries_dispatch_to_selected_handlers(
    test_context *ctx) {
    sprite_status actor;
...
TEST_MAIN_BEGIN;
    test_act31a_table_captures_all_dispatch_slots(&ctx);
    test_act31a_table_entries_dispatch_to_selected_handlers(&ctx);
