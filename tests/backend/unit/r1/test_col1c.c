...
#include "src/r1/col1c.c"

static void reset_col1c_state(void) {
...
static void test_col1c_tables_capture_palette_contract(test_context *ctx) {
    TEST_ASSERT_EQ_INT(ctx, 3,
                       (int)(sizeof(clst1_1c0) / sizeof(clst1_1c0[0])));
    TEST_ASSERT_EQ_INT(ctx, 4,
                       (int)(sizeof(clst1_1c0[0]) / sizeof(clst1_1c0[0][0])));
    TEST_ASSERT_EQ_INT(ctx, 3,
                       (int)(sizeof(clst1_1c1) / sizeof(clst1_1c1[0])));
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone1colorc) / sizeof(zone1colorc[0])));
    TEST_ASSERT_EQ_INT(ctx, 16,
                       (int)(sizeof(zone1colboss1) / sizeof(zone1colboss1[0])));
    TEST_ASSERT_EQ_INT(ctx, 6, (int)(sizeof(colortbl) / sizeof(colortbl[0])));

    assert_palette(ctx, clst1_1c0[0][0], 192, 192, 224, 1, "clst1_1c0");
    assert_palette(ctx, clst1_1c1[2][3], 128, 160, 224, 1, "clst1_1c1");
    ...
    assert_palette(ctx, zone1colorc[47], 160, 160, 0, 1, "zone1colorc[47]");
    assert_palette(ctx, zone1colboss1[15], 224, 0, 0, 1,
                   "zone1colboss1[15]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone1colorc);
...
static void test_clchgctr_updates_two_palette_strips_and_wraps(
    test_context *ctx) {
    reset_col1c_state();
    ...
    assert_palette(ctx, colorwk[53], 128, 160, 224, 1, "strip 53 first");
    assert_palette(ctx, colorwk[56], 160, 192, 224, 1, "strip 56 first");
    assert_palette(ctx, colorwk[44], 128, 160, 224, 1, "strip 44 first");
    assert_palette(ctx, colorwk[47], 160, 192, 224, 1, "strip 47 first");
    ...
    assert_palette(ctx, colorwk[53], 192, 192, 224, 1, "strip 53 wrapped");
    assert_palette(ctx, colorwk[44], 160, 192, 224, 1, "strip 44 wrapped");
}
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 160, 160, 0, 1, "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "colorset3 work 0");
    assert_palette(ctx, colorwk[31], 224, 0, 0, 1, "colorset3 work 0 end");
...
TEST_MAIN_BEGIN;
    test_col1c_tables_capture_palette_contract(&ctx);
    test_clchgctr_updates_two_palette_strips_and_wraps(&ctx);
