...
#include "src/r1/col1a.c"

static void reset_col1a_state(void) {
...
static void test_col1a_tables_capture_palette_contract(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone1colora) / sizeof(zone1colora[0])));
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone1colora0) / sizeof(zone1colora0[0])));
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone1colora1) / sizeof(zone1colora1[0])));
    ...
    assert_palette(ctx, zone1colora[47], 64, 96, 0, 1, "zone1colora[47]");
    assert_palette(ctx, zone1colora0[47], 64, 96, 0, 1, "zone1colora0[47]");
    assert_palette(ctx, zone1colora1[47], 224, 160, 0, 1,
                   "zone1colora1[47]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone1colora1);
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == zone1colora0);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 48, colortbl[5].cnt);

    TEST_ASSERT_EQ_INT(ctx, 49, col1a0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 50, col1a1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 51, col1a2cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 44, col1a3cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 45, col1a4cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 47, col1a5cnt[0]);
    assert_palette(ctx, col1a0col[0], 224, 224, 224, 1, "col1a0col[0]");
    assert_palette(ctx, col1a5col[2], 128, 160, 224, 1, "col1a5col[2]");
}
...
static void test_colchg11a_updates_all_six_cycle_slots(test_context *ctx) {
    reset_col1a_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 6, clchgtim[5]);
    assert_palette(ctx, colorwk[49], 96, 192, 192, 1, "cycle color 49");
    assert_palette(ctx, colorwk[50], 224, 224, 224, 1, "cycle color 50");
    assert_palette(ctx, colorwk[51], 160, 224, 224, 1, "cycle color 51");
    assert_palette(ctx, colorwk[44], 128, 160, 224, 1, "cycle color 44");
    assert_palette(ctx, colorwk[45], 96, 128, 224, 1, "cycle color 45");
    assert_palette(ctx, colorwk[47], 160, 192, 224, 1, "cycle color 47");
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 64, 96, 0, 1, "colorset3 work 0 end");
...
TEST_MAIN_BEGIN;
    test_col1a_tables_capture_palette_contract(&ctx);
    test_clchg_sub0_waits_updates_and_wraps(&ctx);
    test_colchg11a_updates_all_six_cycle_slots(&ctx);
