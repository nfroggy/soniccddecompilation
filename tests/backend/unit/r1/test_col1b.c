...
#include "src/r1/col1b.c"

static void reset_col1b_state(void) {
...
static void test_col1b_tables_capture_palette_contract(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone1colorb) / sizeof(zone1colorb[0])));
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone1colorb0) / sizeof(zone1colorb0[0])));
    ...
    assert_palette(ctx, gamecolor[0], 96, 128, 192, 1, "gamecolor[0]");
    assert_palette(ctx, zone1colorb[47], 224, 192, 96, 1,
                   "zone1colorb[47]");
    assert_palette(ctx, zone1colorb0[47], 32, 160, 96, 1,
                   "zone1colorb0[47]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone1colorb);
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == zone1colorb0);
    ...
    assert_palette(ctx, col1a0col[0], 160, 224, 224, 1, "col1a0col[0]");
    assert_palette(ctx, col1a5col[2], 96, 224, 192, 1, "col1a5col[2]");
}
...
static void test_colchg11b_updates_all_six_cycle_slots(test_context *ctx) {
    reset_col1b_state();
    ...
    assert_palette(ctx, colorwk[49], 96, 224, 192, 1, "cycle color 49");
    assert_palette(ctx, colorwk[50], 160, 224, 224, 1, "cycle color 50");
    assert_palette(ctx, colorwk[51], 64, 192, 128, 1, "cycle color 51");
    assert_palette(ctx, colorwk[44], 96, 224, 192, 1, "cycle color 44");
    assert_palette(ctx, colorwk[45], 64, 192, 128, 1, "cycle color 45");
    assert_palette(ctx, colorwk[47], 192, 224, 224, 1, "cycle color 47");

    reset_col1b_state();
    clchgcnt[0] = 2;
    colchg11b();
    TEST_ASSERT_EQ_INT(ctx, 0, clchgcnt[0]);
    assert_palette(ctx, colorwk[49], 160, 224, 224, 1,
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    colorset(3);
    assert_palette(ctx, colorwk2[0], 96, 128, 192, 1, "colorset work 1");
    assert_palette(ctx, colorwk2[15], 224, 224, 0, 1, "colorset work 1 end");

    colorset2(4);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "colorset2 work 0");
    assert_palette(ctx, colorwk[63], 224, 192, 96, 1, "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "colorset3 work 0");
    assert_palette(ctx, colorwk[63], 32, 160, 96, 1, "colorset3 work 0 end");
...
TEST_MAIN_BEGIN;
    test_col1b_tables_capture_palette_contract(&ctx);
    test_clchg_sub0_waits_updates_and_wraps(&ctx);
    test_colchg11b_updates_all_six_cycle_slots(&ctx);
