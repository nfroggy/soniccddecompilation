...
DECLARE_ACT_STUB(kasoku);
DECLARE_ACT_STUB(hoshi);
DECLARE_ACT_STUB(et);
DECLARE_ACT_STUB(movie1);
DECLARE_ACT_STUB(emie1);
DECLARE_ACT_STUB(heart1);
...
DEFINE_ACT_STUB(kasoku)
DEFINE_ACT_STUB(hoshi)
DEFINE_ACT_STUB(et)
DEFINE_ACT_STUB(movie1)
DEFINE_ACT_STUB(emie1)
DEFINE_ACT_STUB(heart1)
DEFINE_ACT_STUB(ana)
...
static void reset_act11a_state(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_count = 0;
    frameout_actor = 0;
...
static void test_act11a_table_captures_dispatch_slots(test_context *ctx) {
    ...
    assert_act_entry(ctx, 41, kasoku);
    assert_act_entry(ctx, 42, noact);
    assert_act_entry(ctx, 43, hoshi);
    assert_act_entry(ctx, 44, et);
    assert_act_entry(ctx, 45, movie1);
    assert_act_entry(ctx, 46, emie1);
    assert_act_entry(ctx, 47, heart1);
...
static void test_act11a_table_entries_dispatch_to_handlers(test_context *ctx) {
    sprite_status actor;
...
TEST_MAIN_BEGIN;
    test_act11a_table_captures_dispatch_slots(&ctx);
    test_act11a_table_entries_dispatch_to_handlers(&ctx);
