...
static Sint32 vmd_start_call_count;
static Sint32 vmd_menu_call_count;
static Sint32 vmd_draw_call_count;
static Sint32 vmd_end_call_count;
static Sint32 vmd_end_return;
...
#include "src/title/visualmd/vmdentry.c"
...
void VMDStart(void) { ++vmd_start_call_count; }

void VMDMenu(void) { ++vmd_menu_call_count; }

void VMDDraw(void) { ++vmd_draw_call_count; }

Sint32 VMDEnd(void) {
    ++vmd_end_call_count;
    return vmd_end_return;
}
...
static void reset_fixture(void) {
    ...
    vmd_start_call_count = 0;
    vmd_menu_call_count = 0;
    vmd_draw_call_count = 0;
    vmd_end_call_count = 0;
    vmd_end_return = 12;
...
static void test_vmdwait_restores_after_timer_and_handles_key_off(
    test_context *ctx) {
    reset_fixture();
...
static void test_dllinit_binds_imports_sets_globals_and_initializes(
    ...
    TEST_ASSERT_TRUE(ctx, lpbVisualmode == &visual_mode);
...
static void test_swdata_set_copies_both_words(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 0x2468, swData1);
    TEST_ASSERT_EQ_INT(ctx, 0x8642, swData2);
}
...
static void test_dllmain_dispatches_all_sequences(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, vmd_start_call_count);
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, vmd_draw_call_count);

    nSequenceNum = 2;
    TEST_ASSERT_EQ_INT(ctx, 0, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 1, vmd_menu_call_count);
    ...
    nSequenceNum = 999;
    TEST_ASSERT_EQ_INT(ctx, 12, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 1, cd_pause_call_count);
    TEST_ASSERT_EQ_INT(ctx, 2, cd_pause_values[0]);
    TEST_ASSERT_EQ_INT(ctx, 1, vmd_end_call_count);

    nSequenceNum = 777;
    TEST_ASSERT_EQ_INT(ctx, 0, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 5, vmd_draw_call_count);

    nSequenceNum = 1000;
    TEST_ASSERT_EQ_INT(ctx, 0, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 6, vmd_draw_call_count);

    nSequenceNum = 0;
    TEST_ASSERT_EQ_INT(ctx, 0, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 7, vmd_draw_call_count);
}
...
test_vmdwait_restores_after_timer_and_handles_key_off(&ctx);
