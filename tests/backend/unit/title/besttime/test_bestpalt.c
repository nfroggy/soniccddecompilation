...
#include "src/title/besttime/bestpalt.c"
...
static void test_oeclrset_copies_sixteen_entries_to_selected_bank(
    ...
    for (i = 0; i < 16; ++i) {
        assert_palette_entry(ctx, tblPal3[i], palette[32 + i]);
    }
}
...
static void test_oeclrset_uses_source_pointer_not_fixed_tables(
    ...
    reset_palette();
    for (i = 0; i < 16; ++i) {
        custom[i].peRed = (Uint8)(200 - i);
        custom[i].peGreen = (Uint8)(10 + i);
        custom[i].peBlue = (Uint8)(40 + i * 2);
        custom[i].peFlags = (Uint8)(80 + i);
    }
