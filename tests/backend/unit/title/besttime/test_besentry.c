...
static Sint32 fake_cd_play_call_count;
static Sint32 fake_cd_pause_call_count;
static draw_context *draw_context_return;

#include "src/title/besttime/besentry.c"
...
static void fake_cd_play(Sint16 track) {
    (void)track;
    ++fake_cd_play_call_count;
}

static void fake_cd_pause(void) { ++fake_cd_pause_call_count; }
...
static void reset_fixture(void) {
    ...
    stg_end2_return = 7;
    fake_cd_play_call_count = 0;
    fake_cd_pause_call_count = 0;
...
static void test_sequence_wait_helpers_set_saved_state(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, nWaitTime);
...
static void test_dllmain_dispatches_start_menu_wait_and_end(test_context *ctx) {
    reset_fixture();
    ...
    nSequenceNum = 998;
    TEST_ASSERT_EQ_INT(ctx, 7, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 1, stg_end2_call_count);

    nSequenceNum = 777;
    TEST_ASSERT_EQ_INT(ctx, 0, DLLMain());
    TEST_ASSERT_EQ_INT(ctx, 5, stg_draw_call_count);
...
test_dllmain_dispatches_start_menu_wait_and_end(&ctx);
