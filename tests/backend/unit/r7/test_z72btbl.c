...
static void test_z72btbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 3838821926u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 1413095735u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 3060506013u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z72btbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z72btbl_map_anchors(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 2, mapwka[0][2]);
    TEST_ASSERT_EQ_INT(ctx, 12, mapwka[0][3]);
    TEST_ASSERT_EQ_INT(ctx, 129, mapwka[0][27]);
    ...
    TEST_ASSERT_EQ_INT(ctx, 102, mapwkb[0][2]);
    TEST_ASSERT_EQ_INT(ctx, 106, mapwkb[1][0]);
    TEST_ASSERT_EQ_INT(ctx, 114, mapwkb[2][2]);
    TEST_ASSERT_EQ_INT(ctx, 0, mapwkb[7][63]);
}

TEST_MAIN_BEGIN;
    test_z72btbl_table_sizes_and_fingerprints(&ctx);
    test_z72btbl_direction_and_collision_anchors(&ctx);
    test_z72btbl_map_anchors(&ctx);
TEST_MAIN_END
