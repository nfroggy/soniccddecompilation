...
#include "src/r7/col7c.c"

static void reset_col7c_state(void) {
...
static void test_col7c_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    assert_palette(ctx, advacolor[33], 128, 128, 0, 1, "advacolor[33]");
    assert_palette(ctx, advacolor[47], 224, 192, 0, 1, "advacolor[47]");
    assert_palette(ctx, advacolor[63], 224, 128, 192, 1, "advacolor[63]");
    assert_palette(ctx, gamecolor[32], 0, 32, 0, 1, "gamecolor[32]");
    assert_palette(ctx, gamecolor[63], 224, 32, 96, 1, "gamecolor[63]");
    assert_palette(ctx, zone82colora[16], 0, 32, 0, 1,
                   "zone82colora[16]");
    assert_palette(ctx, zone82colora[47], 224, 32, 96, 1,
                   "zone82colora[47]");
...
static void test_clchgctr_is_empty(test_context *ctx) {
    reset_col7c_state();
    clchgtim[0] = 12;
    clchgcnt[0] = 34;
    colorwk[41] = (PALETTEENTRY){1, 2, 3, 4};

    clchgctr();

    TEST_ASSERT_EQ_INT(ctx, 12, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 34, clchgcnt[0]);
    assert_palette(ctx, colorwk[41], 1, 2, 3, 4, "empty clchgctr color");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 224, 32, 96, 1,
                   "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk2[16], 0, 0, 0, 1, "colorset3 work 1");
    assert_palette(ctx, colorwk2[63], 224, 32, 96, 1,
                   "colorset3 work 1 end");

    colorset4(5);
    assert_palette(ctx, colorwk3[16], 0, 0, 0, 1, "colorset4 work 2");
    assert_palette(ctx, colorwk3[63], 224, 32, 96, 1,
                   "colorset4 work 2 end");

    colorset00(6, 0);
    assert_palette(ctx, colorwk[16], 32, 32, 32, 1, "colorset00 boss");
    assert_palette(ctx, colorwk[21], 0, 224, 0, 1, "colorset00 boss end");

    colorset00(0, 3);
    assert_palette(ctx, colorwk4[0], 0, 0, 0, 1, "colorset00 default");
    assert_palette(ctx, colorwk4[63], 224, 128, 192, 1,
...
TEST_MAIN_BEGIN;
test_col7c_tables_capture_literal_palette_data(&ctx);
test_clchgctr_is_empty(&ctx);
