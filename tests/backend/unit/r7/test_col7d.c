...
#include "src/r7/col7d.c"

static void reset_col7d_state(void) {
...
static void test_col7d_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 6,
                       (int)(sizeof(bosscolor) / sizeof(bosscolor[0])));
    TEST_ASSERT_EQ_INT(ctx, 7, (int)(sizeof(colortbl) / sizeof(colortbl[0])));

    assert_palette(ctx, advacolor[0], 0, 0, 0, 1, "advacolor[0]");
    assert_palette(ctx, advacolor[33], 224, 224, 64, 1, "advacolor[33]");
    assert_palette(ctx, advacolor[63], 160, 192, 160, 1, "advacolor[63]");
    ...
    assert_palette(ctx, zone8colora[16], 32, 0, 0, 1, "zone8colora[16]");
    assert_palette(ctx, zone8colora[47], 64, 160, 128, 1,
                   "zone8colora[47]");
    assert_palette(ctx, bosscolor[0], 32, 32, 32, 1, "bosscolor[0]");
    assert_palette(ctx, bosscolor[5], 0, 224, 0, 1, "bosscolor[5]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == 0);
    TEST_ASSERT_TRUE(ctx, colortbl[6].PeSrc == bosscolor);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 0, colortbl[5].cnt);
    TEST_ASSERT_EQ_INT(ctx, 6, colortbl[6].cnt);
}
...
static void test_colchg7d_updates_stage_zero_cycle_slots(test_context *ctx) {
    reset_col7d_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 35, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 4, clchgtim[1]);
    TEST_ASSERT_EQ_INT(ctx, 4, clchgtim[2]);
    TEST_ASSERT_EQ_INT(ctx, 4, clchgtim[3]);
    TEST_ASSERT_EQ_INT(ctx, 100, clchgtim[4]);
    TEST_ASSERT_EQ_INT(ctx, 100, clchgtim[5]);
    assert_palette(ctx, colorwk[39], 0, 0, 0, 1, "cycle color 39");
    assert_palette(ctx, colorwk[45], 192, 64, 64, 1, "cycle color 45");
    assert_palette(ctx, colorwk[46], 96, 0, 0, 1, "cycle color 46");
    assert_palette(ctx, colorwk[47], 192, 128, 128, 1, "cycle color 47");
    assert_palette(ctx, colorwk[49], 32, 0, 0, 1, "cycle color 49");
    assert_palette(ctx, colorwk[62], 32, 0, 0, 1, "cycle color 62");
}

static void test_colchg7d_uses_alternate_last_slot_when_stage_low_nonzero(
    test_context *ctx) {
    reset_col7d_state();
    stageno.b.l = 1;
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[5]);
    TEST_ASSERT_EQ_INT(ctx, 100, clchgtim[5]);
    assert_palette(ctx, colorwk[58], 32, 0, 0, 1, "alternate cycle color 58");
    assert_palette(ctx, colorwk[62], 0, 0, 0, 0, "skipped cycle color 62");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 64, 160, 128, 1,
                   "colorset2 work 0 end");

    colorset3(4);
    assert_palette(ctx, colorwk2[16], 0, 0, 0, 1, "colorset3 work 1");
    assert_palette(ctx, colorwk2[63], 64, 160, 128, 1,
                   "colorset3 work 1 end");

    colorset4(6);
    assert_palette(ctx, colorwk3[16], 32, 32, 32, 1, "colorset4 boss");
    assert_palette(ctx, colorwk3[21], 0, 224, 0, 1, "colorset4 boss end");

    colorset00(2, 0);
    assert_palette(ctx, colorwk[49], 192, 224, 224, 1,
                   "colorset00 advacolor2");

    colorset00(0, 3);
    assert_palette(ctx, colorwk4[0], 0, 0, 0, 1, "colorset00 default");
    assert_palette(ctx, colorwk4[63], 160, 192, 160, 1,
...
TEST_MAIN_BEGIN;
test_col7d_tables_capture_literal_palette_data(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_colchg7d_updates_stage_zero_cycle_slots(&ctx);
test_colchg7d_uses_alternate_last_slot_when_stage_low_nonzero(&ctx);
