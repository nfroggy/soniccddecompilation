...
static void test_z71ctbl_table_sizes_and_fingerprints(test_context *ctx) {
    ...
    assert_eq_u32(ctx, 2204738079u, fnv1a32(scdtblwk2, sizeof(scdtblwk2)),
                  "scdtblwk2 fnv1a32");
    assert_eq_u32(ctx, 2564153712u, fnv1a32(zone1scd, sizeof(zone1scd)),
                  "zone1scd fnv1a32");
    assert_eq_u32(ctx, 3105919215u, fnv1a32(mapwka, sizeof(mapwka)),
                  "mapwka fnv1a32");
    assert_eq_u32(ctx, 2059497974u, fnv1a32(mapwkb, sizeof(mapwkb)),
                  "mapwkb fnv1a32");
}

static void test_z71ctbl_direction_and_collision_anchors(test_context *ctx) {
...
static void test_z71ctbl_map_anchors(test_context *ctx) {
    TEST_ASSERT_EQ_INT(ctx, 0, zone1scd[0]);
    TEST_ASSERT_EQ_INT(ctx, 81, zone1scd[1]);
    TEST_ASSERT_EQ_INT(ctx, 122, zone1scd[13]);
    TEST_ASSERT_EQ_INT(ctx, 106, zone1scd[46]);
    TEST_ASSERT_EQ_INT(ctx, 0, zone1scd[999]);

    TEST_ASSERT_EQ_INT(ctx, 27, mapwka[0][0]);
    TEST_ASSERT_EQ_INT(ctx, 167, mapwka[0][2]);
    TEST_ASSERT_EQ_INT(ctx, 129, mapwka[0][9]);
...
TEST_MAIN_BEGIN;
    test_z71ctbl_table_sizes_and_fingerprints(&ctx);
    test_z71ctbl_direction_and_collision_anchors(&ctx);
    test_z71ctbl_map_anchors(&ctx);
TEST_MAIN_END
