...
void marker(sprite_status *pActwk);
...
void ball(sprite_status *pActwk);
...
static void reset_act71a_state(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_count = 0;
    frameout_actor = 0;
...
static void test_act71a_table_captures_actor_dispatch_order(test_context *ctx) {
    ...
    TEST_ASSERT_TRUE(ctx, act_tbl[7] == gun7);
    TEST_ASSERT_TRUE(ctx, act_tbl[8] == hibana);
    ...
    TEST_ASSERT_TRUE(ctx, act_tbl[18] == marker);
    ...
    TEST_ASSERT_TRUE(ctx, act_tbl[28] == chgban);
    TEST_ASSERT_TRUE(ctx, act_tbl[29] == spring_r);
    TEST_ASSERT_TRUE(ctx, act_tbl[30] == flower);
    TEST_ASSERT_TRUE(ctx, act_tbl[31] == branko7);
    TEST_ASSERT_TRUE(ctx, act_tbl[32] == kanabun);
    TEST_ASSERT_TRUE(ctx, act_tbl[33] == kabasira);
    TEST_ASSERT_TRUE(ctx, act_tbl[34] == hotaru7);
    ...
    TEST_ASSERT_TRUE(ctx, act_tbl[40] == et);
    TEST_ASSERT_TRUE(ctx, act_tbl[41] == dango);
    TEST_ASSERT_TRUE(ctx, act_tbl[42] == ball);
    TEST_ASSERT_TRUE(ctx, act_tbl[43] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[44] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[45] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[46] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[47] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[48] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[49] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[50] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[51] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[52] == chgwall7);
    TEST_ASSERT_TRUE(ctx, act_tbl[53] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[54] == noact);
    TEST_ASSERT_TRUE(ctx, act_tbl[55] == slight7);
...
TEST_MAIN_BEGIN;
    test_act71a_table_captures_actor_dispatch_order(&ctx);
