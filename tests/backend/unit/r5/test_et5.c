...
static void reset_et5_state(void) {
...
static void reset_et5_logs(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_s_count = 0;
    frameout_s_actor = 0;
...
static void test_et5_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 422, et_pat00.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, 428, et_pat06.spra[0].index);
...
static void test_et5_entry_initializes_default_and_waits(test_context *ctx) {
...
static void test_et5_init_variants_capture_pattern_and_wait_gates(
    test_context *ctx) {
    sprite_status *actor = &actwk[4];
...
static void initialize_breaking_et(sprite_status *actor) {
    actor->xposi.w.h = 100;
    actor->yposi.w.h = 200;
    et(actor);
    reset_et5_logs();
...
static void test_et5_entry_dispatches_die_and_reset_states(test_context *ctx) {
    sprite_status *actor = &actwk[4];
    int i;
...
TEST_MAIN_BEGIN;
    test_et5_tables_capture_literal_data(&ctx);
    test_et5_entry_initializes_default_and_waits(&ctx);
    test_et5_init_variants_capture_pattern_and_wait_gates(&ctx);
    ...
    test_et5_entry_dispatches_die_and_reset_states(&ctx);
