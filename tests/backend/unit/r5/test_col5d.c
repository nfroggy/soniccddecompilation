...
#include "src/r5/col5d.c"

static void reset_col5d_state(void) {
...
static void test_col5d_tables_capture_palette_contract(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 16,
                       (int)(sizeof(zone5colboss0) / sizeof(zone5colboss0[0])));
    ...
    assert_palette(ctx, zone5color0[47], 224, 160, 0, 1,
                   "zone5color0[47]");
    assert_palette(ctx, zone5color1[47], 224, 224, 0, 1,
                   "zone5color1[47]");
    assert_palette(ctx, zone5colboss0[15], 224, 0, 0, 1,
                   "zone5colboss0[15]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[6].PeSrc == zone5colboss0);
...
static void test_clchgctr_updates_palette_cycles(test_context *ctx) {
    reset_col5d_state();
    ...
    assert_palette(ctx, colorwk[42], 0, 0, 0, 1, "normal color 42");
    assert_palette(ctx, colorwk[43], 224, 224, 192, 1, "normal color 43");
    assert_palette(ctx, colorwk[44], 96, 96, 64, 1, "normal color 44");

    reset_col5d_state();
    colrevflag = 1;
    clchgctr();

    assert_palette(ctx, colorwk[42], 160, 0, 128, 1, "reverse color 42");
    assert_palette(ctx, colorwk[43], 64, 0, 96, 1, "reverse color 43");
    assert_palette(ctx, colorwk[44], 224, 0, 160, 1, "reverse color 44");
    ...
    TEST_ASSERT_EQ_INT(ctx, 12, clchgtim[1]);
    TEST_ASSERT_EQ_INT(ctx, 5, clchgtim[2]);
    assert_palette(ctx, colorwk[54], 160, 32, 64, 1, "enkei color 54");
    assert_palette(ctx, colorwk[57], 224, 96, 64, 1, "enkei color 57");
    assert_palette(ctx, colorwk[60], 128, 128, 0, 1, "enkei color 60");
    assert_palette(ctx, colorwk[63], 224, 224, 0, 1, "enkei color 63");

    reset_col5d_state();
    bossflag = 1;
    enkeino = 1;
    colorwk[42] = (PALETTEENTRY){7, 8, 9, 10};
    clchgctr();

    TEST_ASSERT_EQ_INT(ctx, 0, clchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 0, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[1]);
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[2]);
    assert_palette(ctx, colorwk[42], 7, 8, 9, 10, "boss skipped color");
    assert_palette(ctx, colorwk[54], 160, 32, 64, 1,
                   "boss enkei still updates");
}

static void test_fadein_and_fadeout_paths(test_context *ctx) {
    reset_col5d_state();
    startcolor = 4;
    colorcnt = 2;
...
static void test_flashin_and_flashout_paths(test_context *ctx) {
    reset_col5d_state();
...
static void test_direct_helpers_and_colorsets(test_context *ctx) {
...
TEST_MAIN_BEGIN;
    test_col5d_tables_capture_palette_contract(&ctx);
    test_colchgsub_waits_updates_and_wraps(&ctx);
    test_clchgctr_updates_palette_cycles(&ctx);
    test_fadein_and_fadeout_paths(&ctx);
    test_flashin_and_flashout_paths(&ctx);
    test_direct_helpers_and_colorsets(&ctx);
