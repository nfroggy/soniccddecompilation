...
static void test_coli5_tables_capture_r5_collision_contract(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 30, colitbl[62][0]);
    TEST_ASSERT_EQ_INT(ctx, 32, colitbl[62][1]);
...
static void test_item_collision_preserves_bumper_behaviors(test_context *ctx) {
    ...
    reset_coli5_state();
    item->colino = 65;
    set_player_invincibility_timer(player, 90);
    TEST_ASSERT_EQ_INT(ctx, -1, pcolitem(player, item));
...
static void test_player_damage_and_death_paths(test_context *ctx) {
    ...
    reset_coli5_state();
    set_player_invincibility_timer(player, 1);
...
static void test_special_yago_boss_and_main_attack_paths(test_context *ctx) {
    sprite_status *player = &actwk[0];
    sprite_status *target = &actwk[1];
    ...
    reset_coli5_state();
    target->colino = 192 + 55;
    TEST_ASSERT_EQ_INT(ctx, -1, pcolspecial(player, target, 0, 0, 16));
    TEST_ASSERT_EQ_INT(ctx, 1, target->colicnt);

    reset_coli5_state();
    target->colino = 192 + 56;
    plpower_m = 1;
    TEST_ASSERT_EQ_INT(ctx, -1, pcolspecial(player, target, 0, 0, 16));
    TEST_ASSERT_EQ_INT(ctx, 1, target->colicnt);
    ...
    reset_coli5_state();
    plpower_a = 1;
    TEST_ASSERT_EQ_INT(ctx, -1, yago(player, target, 0, 0, 16));

    reset_coli5_state();
    TEST_ASSERT_EQ_INT(ctx, -1, yago(player, target, 100, 12, 16));

    reset_coli5_state();
    target->xposi.w.h = 100;
    target->cddat = 1;
    TEST_ASSERT_EQ_INT(ctx, -1, yago(player, target, 80, 12, 16));

    reset_coli5_state();
    target->xposi.w.h = 100;
    plpower_m = 1;
    TEST_ASSERT_EQ_INT(ctx, -1, yago(player, target, 110, 12, 16));
...
TEST_MAIN_BEGIN;
    test_coli5_tables_capture_r5_collision_contract(&ctx);
    ...
    test_special_yago_boss_and_main_attack_paths(&ctx);
