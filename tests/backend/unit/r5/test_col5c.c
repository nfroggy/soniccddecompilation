...
#include "src/r5/col5c.c"

static void reset_col5c_state(void) {
...
static void test_col5c_tables_capture_palette_contract(test_context *ctx) {
    ...
    assert_palette(ctx, zone5color0[47], 192, 128, 224, 1,
                   "zone5color0[47]");
    assert_palette(ctx, zone5color1[47], 0, 0, 0, 1, "zone5color1[47]");
...
static void test_clchgctr_updates_normal_reverse_and_boss_skip(
    test_context *ctx) {
    reset_col5c_state();
    ...
    assert_palette(ctx, colorwk[42], 32, 192, 96, 1, "normal color 42");
    assert_palette(ctx, colorwk[43], 0, 32, 0, 1, "normal color 43");
    assert_palette(ctx, colorwk[44], 0, 128, 0, 1, "normal color 44");
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 4, clchgtim[0]);
    assert_palette(ctx, colorwk[42], 128, 0, 160, 1, "reverse color 42");
    assert_palette(ctx, colorwk[43], 224, 0, 224, 1, "reverse color 43");
    assert_palette(ctx, colorwk[44], 64, 0, 64, 1, "reverse color 44");
...
static void test_colorset_routes_to_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 192, 128, 224, 1,
                   "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk2[16], 0, 0, 0, 1, "colorset3 work 1");
    assert_palette(ctx, colorwk2[63], 0, 0, 0, 1, "colorset3 work 1 end");
...
TEST_MAIN_BEGIN;
    test_col5c_tables_capture_palette_contract(&ctx);
    test_colchgsub_waits_updates_and_wraps(&ctx);
    test_clchgctr_updates_normal_reverse_and_boss_skip(&ctx);
