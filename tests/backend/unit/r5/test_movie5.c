...
static void reset_movie5_state(void) {
...
static void test_movie5_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 255, pchg01[57]);

    TEST_ASSERT_TRUE(ctx, pat_movie[0] == &movie_pat00);
    TEST_ASSERT_TRUE(ctx, pat_movie[1] == &movie_pat01);
    TEST_ASSERT_TRUE(ctx, pat_movie[5] == &movie_pat05);
    TEST_ASSERT_EQ_INT(ctx, -12, movie_pat00.spra[0].xoff);
    TEST_ASSERT_EQ_INT(ctx, -12, movie_pat00.spra[0].yoff);
    TEST_ASSERT_EQ_INT(ctx, 445, movie_pat00.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, -20, movie_pat04.spra[0].xoff);
    TEST_ASSERT_EQ_INT(ctx, -24, movie_pat04.spra[0].yoff);
    TEST_ASSERT_EQ_INT(ctx, 448, movie_pat04.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, 449, movie_pat05.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, 1, tbl0[0]);
    TEST_ASSERT_EQ_INT(ctx, -1, tbl0[36]);
}

static void test_projector_flag_frameouts_main_actor(test_context *ctx) {
...
static void test_movie_initializes_and_spawns_four_children(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 950, actor->sproffset);
    TEST_ASSERT_TRUE(ctx, actor->patbase == pat_movie);

    TEST_ASSERT_EQ_INT(ctx, 44, actwk[32].actno);
    ...
    TEST_ASSERT_EQ_INT(ctx, 45, actwk[34].actno);
    ...
    TEST_ASSERT_EQ_INT(ctx, 45, actwk[35].actno);
    TEST_ASSERT_EQ_INT(ctx, 300, actwk[35].xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 204, actwk[35].yposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, -127, actwk[35].userflag.b.h);
}

static void test_allocation_failures_frameout_at_each_child(
    test_context *ctx) {
    int fail_at;
...
static void test_wait_without_collision_calls_hitchk(test_context *ctx) {
    sprite_status *actor = &actwk[2];
...
static void test_collision_enters_destruction_and_clears_ride(
    test_context *ctx) {
    sprite_status *actor = &actwk[2];
...
static void test_destruction_skips_ride_clear_when_not_hitched(
    test_context *ctx) {
    sprite_status *actor = &actwk[2];
...
static void test_explosion_allocator_failure_skips_spawn(test_context *ctx) {
    sprite_status *actor = &actwk[2];
...
static void test_explosion_sequence_finishes_and_sets_projector_flag(
    test_context *ctx) {
    sprite_status *actor = &actwk[2];
    int i;
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, projector_flag);
    TEST_ASSERT_EQ_INT(ctx, 1, frameout_count);
    TEST_ASSERT_TRUE(ctx, frameout_actor == actor);
}

static void test_child_frameouts_when_parent_missing_or_destroying(
    test_context *ctx) {
...
static void test_child_initializes_negative_and_positive_shapes(
    test_context *ctx) {
    sprite_status *child = &actwk[5];
    sprite_status *parent = &actwk[2];
    ...
    TEST_ASSERT_EQ_INT(ctx, 950, child->sproffset);
...
static void test_child_moves_with_patch_animation(test_context *ctx) {
    sprite_status *child = &actwk[5];
    sprite_status *parent = &actwk[2];
...
TEST_MAIN_BEGIN;
    test_movie5_tables_capture_literal_data(&ctx);
    test_projector_flag_frameouts_main_actor(&ctx);
    test_movie_initializes_and_spawns_four_children(&ctx);
    test_allocation_failures_frameout_at_each_child(&ctx);
    test_wait_without_collision_calls_hitchk(&ctx);
    test_collision_enters_destruction_and_clears_ride(&ctx);
    test_destruction_skips_ride_clear_when_not_hitched(&ctx);
    test_explosion_allocator_failure_skips_spawn(&ctx);
    test_explosion_sequence_finishes_and_sets_projector_flag(&ctx);
    test_child_frameouts_when_parent_missing_or_destroying(&ctx);
    test_child_initializes_negative_and_positive_shapes(&ctx);
    test_child_moves_with_patch_animation(&ctx);
TEST_MAIN_END
