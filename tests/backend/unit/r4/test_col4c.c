...
#include "src/r4/col4c.c"

static void reset_col4c_state(void) {
...
static void test_col4c_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[5].PeSrc == boss4color);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[5].offset);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[5].cnt);

    TEST_ASSERT_EQ_INT(ctx, 49, col4c0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 50, col4c1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 51, col4c2cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 52, col4c3cnt[0]);
    assert_palette(ctx, col4c0col[0], 192, 192, 192, 1, "col4c0col[0]");
    assert_palette(ctx, col4c0col[3], 160, 192, 224, 1, "col4c0col[3]");
}
...
static void test_clchgctr_only_updates_when_stage_low_byte_set(
    test_context *ctx) {
    reset_col4c_state();
    ...
    assert_palette(ctx, colorwk[50], 0, 0, 0, 0, "no stage cycle");
    ...
    assert_palette(ctx, colorwk[50], 96, 160, 224, 1, "cycle color 50");
    assert_palette(ctx, colorwk[51], 160, 192, 224, 1, "cycle color 51");
    assert_palette(ctx, colorwk[52], 192, 192, 192, 1, "cycle color 52");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 160, 128, 192, 1, "colorset2 work 0 end");

    colorset3(5);
    assert_palette(ctx, colorwk3[16], 0, 0, 0, 1, "colorset3 boss");
    assert_palette(ctx, colorwk3[31], 224, 32, 0, 1, "colorset3 boss end");
...
TEST_MAIN_BEGIN;
test_col4c_tables_capture_literal_palette_data(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_clchgctr_only_updates_when_stage_low_byte_set(&ctx);
