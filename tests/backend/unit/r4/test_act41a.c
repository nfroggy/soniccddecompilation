...
static void test_debugact_advances_coordinates_and_draws(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 13, actor.xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 35, actor.yposi.w.h);
...
static void test_dispatch_table_preserves_existing_slots(test_context *ctx) {
    sprite_status actor;
    reset_state(&actor);
    ...
    TEST_ASSERT_TRUE(ctx, act_tbl[48] == &switchr4);
...
test_dispatch_table_preserves_existing_slots(&ctx);
TEST_MAIN_END
