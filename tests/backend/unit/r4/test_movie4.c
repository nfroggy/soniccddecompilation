...
static void test_movie4_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, SPRITE_MOVIE4_BASE, spr_movie1.spra[0].index);
    TEST_ASSERT_EQ_INT(ctx, SPRITE_MOVIE4_BASE + 4, spr_movie5.spra[0].index);
...
static void test_mm_init_projector_and_allocation_paths(test_context *ctx) {
    ...
    TEST_ASSERT_TRUE(ctx, movie4_work_get(main)->script == tbl0);
}
...
static void test_mm_init_spawns_four_children(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 3, movie4_work_get(&actwk[40])->parent_actor);
...
static void test_mm_wait_and_die_paths(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 255, movie4_work_get(main)->parent_destroyed);
...
static void test_m_baku_waits_spawns_and_finishes_script(test_context *ctx) {
    ...
    main->xposi.w.h = 400;
    main->yposi.w.h = 200;
    movie4_work_get(main)->script = tbl0;
    queue_actor(&actwk[40]);
    ...
    TEST_ASSERT_TRUE(ctx, movie4_work_get(main)->script == &tbl0[3]);
    ...
    reset_movie4_state();
    main->r_no0 = 6;
    movie4_work_get(main)->script = &tbl0[36];
    m_baku(main);
    TEST_ASSERT_EQ_INT(ctx, 8, main->r_no0);
    TEST_ASSERT_EQ_INT(ctx, 60, movie4_work_get(main)->wait_timer);
}
...
static void test_mm1wait_counts_down_and_sets_projector(test_context *ctx) {
    ...
    reset_movie4_state();
    movie4_work_get(main)->wait_timer = 2;
    mm1wait(main);
    TEST_ASSERT_EQ_INT(ctx, 1, movie4_work_get(main)->wait_timer);
...
static void test_sub_parent_gates_and_initializes_variants(test_context *ctx) {
    ...
    reset_movie4_state();
    movie4_work_get(child)->parent_actor = 3;
    parent->actno = 0;
    sub(child);
    TEST_ASSERT_EQ_INT(ctx, 1, frameout_count);
    ...
    movie4_work_get(child)->parent_actor = 3;
    parent->actno = 82;
    movie4_work_get(parent)->parent_destroyed = 1;
    sub(child);
    TEST_ASSERT_EQ_INT(ctx, 1, frameout_count);
