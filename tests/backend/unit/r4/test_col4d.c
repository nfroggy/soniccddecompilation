...
#include "src/r4/col4d.c"

static void reset_col4d_state(void) {
...
static void test_col4d_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    assert_palette(ctx, advacolor[63], 96, 32, 32, 1, "advacolor[63]");
    assert_palette(ctx, advacolor2[33], 224, 224, 0, 1, "advacolor2[33]");
    assert_palette(ctx, gamecolor[15], 224, 224, 0, 1, "gamecolor[15]");
    assert_palette(ctx, zone4colora[23], 192, 96, 0, 1, "zone4colora[23]");
    ...
    TEST_ASSERT_EQ_INT(ctx, 50, col4d0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 51, col4d1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 52, col4d2cnt[0]);
    assert_palette(ctx, col4d0col[0], 160, 96, 192, 1, "col4d0col[0]");
    assert_palette(ctx, col4d0col[2], 224, 160, 192, 1, "col4d0col[2]");
}
...
static void test_clchgctr_only_updates_when_stage_low_byte_set(
    ...
    assert_palette(ctx, colorwk[50], 192, 128, 192, 1, "cycle color 50");
    assert_palette(ctx, colorwk[51], 224, 160, 192, 1, "cycle color 51");
    assert_palette(ctx, colorwk[52], 160, 96, 192, 1, "cycle color 52");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 128, 64, 0, 1, "colorset2 work 0 end");
...
TEST_MAIN_BEGIN;
test_col4d_tables_capture_literal_palette_data(&ctx);
