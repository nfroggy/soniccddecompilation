...
#include "src/r4/col4b.c"

static void reset_col4b_state(void) {
...
static void test_col4b_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    assert_palette(ctx, advacolor[63], 192, 128, 0, 1, "advacolor[63]");
    ...
    TEST_ASSERT_EQ_INT(ctx, 50, col4b0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 51, col4b1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 52, col4b2cnt[0]);
    assert_palette(ctx, col4b0col[0], 0, 192, 192, 1, "col4b0col[0]");
    assert_palette(ctx, col4b0col[2], 64, 224, 224, 1, "col4b0col[2]");
}
...
static void test_clchgctr_only_updates_when_stage_low_byte_set(
    ...
    assert_palette(ctx, colorwk[50], 0, 224, 224, 1, "cycle color 50");
    assert_palette(ctx, colorwk[51], 64, 224, 224, 1, "cycle color 51");
    assert_palette(ctx, colorwk[52], 0, 192, 192, 1, "cycle color 52");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 224, 192, 64, 1, "colorset2 work 0 end");
...
TEST_MAIN_BEGIN;
test_col4b_tables_capture_literal_palette_data(&ctx);
