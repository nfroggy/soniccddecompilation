...
static void test_debugact_wraps_existing_threshold_values(test_context *ctx) {
    sprite_status actor;
    reset_state(&actor);
    actor.xposi.w.h = 301;
    actor.yposi.w.h = 201;
...
static void test_dispatch_table_preserves_r41b_slots(test_context *ctx) {
    sprite_status actor;
    reset_state(&actor);
...
test_debugact_wraps_existing_threshold_values(&ctx);
test_noact_frames_actor_out(&ctx);
test_dispatch_table_preserves_r41b_slots(&ctx);
TEST_MAIN_END
