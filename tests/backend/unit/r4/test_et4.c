...
static void reset_et4_state(void) {
...
static void reset_et4_logs(void) {
    actionsub_count = 0;
    actionsub_actor = 0;
    frameout_s_count = 0;
    frameout_s_actor = 0;
...
static void prepare_actor_for_die(sprite_status *actor) {
    actor->xposi.w.h = 100;
    actor->yposi.w.h = 200;
    actor->colicnt = 5;
    et(actor);
...
static void test_et4_tables_capture_literal_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, -40, tbl0[4]);
    TEST_ASSERT_EQ_INT(ctx, -1, tbl0[63]);
}

static void test_et4_entry_initializes_default_and_waits(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 5, et4_work_get(actor)->hover_counter);
    TEST_ASSERT_EQ_INT(ctx, 200, et4_work_get(actor)->base_y);
    TEST_ASSERT_EQ_INT(ctx, 1, et4_work_get(actor)->hover_direction);
...
static void test_et4_init_variants_capture_pattern_and_wait_gates(
    test_context *ctx) {
    sprite_status *actor = &actwk[4];
    ...
    actor->yposi.w.h = 340;
    et(actor);
    TEST_ASSERT_EQ_INT(ctx, 0, actor->sproffset);
}

static void test_m_wait_collision_scores_and_optionally_clears_ride(
    test_context *ctx) {
    sprite_status *actor = &actwk[4];
    ...
    actor->colicnt = 5;
    actor->patno = 3;
    et4_work_get(actor)->hover_counter = 9;
    hitchk_result = 0;
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, et4_work_get(actor)->hover_counter);
...
static void test_m_die_waits_for_script_tick_then_spawns_fragment(
    ...
    reset_et4_state();
    prepare_actor_for_die(actor);
    et4_work_get(actor)->delay_timer = 1;

    m_die(actor);

    TEST_ASSERT_EQ_INT(ctx, 2, et4_work_get(actor)->delay_timer);
    ...
    TEST_ASSERT_EQ_INT(ctx, 184, fragment->yposi.w.h);
...
static void test_m_die_handles_allocation_failure_and_script_end(
    ...
    TEST_ASSERT_EQ_INT(ctx, 8, et4_work_get(actor)->delay_timer);
}
...
static void test_m1wait_counts_down_then_restores_origin(test_context *ctx) {
    ...
    reset_et4_logs();
    actor->r_no0 = 6;
    et4_work_get(actor)->delay_timer = 2;
    actor->yposi.w.h = 77;

    m1wait(actor);

    TEST_ASSERT_EQ_INT(ctx, 6, actor->r_no0);
    TEST_ASSERT_EQ_INT(ctx, 1, et4_work_get(actor)->delay_timer);
    ...
    TEST_ASSERT_EQ_INT(ctx, 0, et4_work_get(actor)->delay_timer);
...
static void test_a_hover_moves_every_eight_ticks_and_flips_every_thirty_two(
    ...
    reset_et4_state();
    actor->yposi.w.h = 100;
    et4_work_get(actor)->hover_counter = 7;
    et4_work_get(actor)->hover_direction = 2;

    a_hover(actor);

    TEST_ASSERT_EQ_INT(ctx, 8, et4_work_get(actor)->hover_counter);
    TEST_ASSERT_EQ_INT(ctx, 102, actor->yposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 2, et4_work_get(actor)->hover_direction);

    et4_work_get(actor)->hover_counter = 31;
    et4_work_get(actor)->hover_direction = 3;

    a_hover(actor);

    TEST_ASSERT_EQ_INT(ctx, 32, et4_work_get(actor)->hover_counter);
    TEST_ASSERT_EQ_INT(ctx, 105, actor->yposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, -3, et4_work_get(actor)->hover_direction);
}

TEST_MAIN_BEGIN;
test_et4_tables_capture_literal_data(&ctx);
test_et4_entry_initializes_default_and_waits(&ctx);
test_et4_init_variants_capture_pattern_and_wait_gates(&ctx);
test_m_wait_collision_scores_and_optionally_clears_ride(&ctx);
