...
DECLARE_ACT_STUB(rblk4);
DECLARE_ACT_STUB(et);
DECLARE_ACT_STUB(movie4);

#include "src/r4/act42b.c"
...
DEFINE_ACT_STUB(rblk4)
DEFINE_ACT_STUB(et)
DEFINE_ACT_STUB(movie4)
...
static void test_debugact_advances_coordinates_and_draws(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 121, actor.xposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 81, actor.yposi.w.h);
    TEST_ASSERT_EQ_INT(ctx, 1, actionsub_count);
    TEST_ASSERT_TRUE(ctx, actionsub_actor == &actor);
}

static void test_debugact_wraps_past_limits(test_context *ctx) {
    sprite_status actor;
    reset_state(&actor);
    actor.xposi.w.h = 32767;
    actor.yposi.w.h = 32767;
...
static void test_dispatch_table_preserves_r42b_slots(test_context *ctx) {
    sprite_status actor;
    reset_state(&actor);
    ...
    TEST_ASSERT_TRUE(ctx, act_tbl[50] == &walls);
    TEST_ASSERT_TRUE(ctx, act_tbl[56] == &kuzure4);
    TEST_ASSERT_TRUE(ctx, act_tbl[60] == &rblk4);
    TEST_ASSERT_TRUE(ctx, act_tbl[80] == &et);
    TEST_ASSERT_TRUE(ctx, act_tbl[81] == &movie4);
...
test_debugact_wraps_past_limits(&ctx);
test_noact_frames_actor_out(&ctx);
test_dispatch_table_preserves_r42b_slots(&ctx);
TEST_MAIN_END
