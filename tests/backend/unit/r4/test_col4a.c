...
#include "src/r4/col4a.c"

static void reset_col4a_state(void) {
...
static void test_col4a_tables_capture_literal_palette_data(test_context *ctx) {
    ...
    TEST_ASSERT_EQ_INT(ctx, 48,
                       (int)(sizeof(zone4colora) / sizeof(zone4colora[0])));
    TEST_ASSERT_EQ_INT(ctx, 5, (int)(sizeof(colortbl) / sizeof(colortbl[0])));

    assert_palette(ctx, advacolor[0], 0, 0, 0, 1, "advacolor[0]");
    assert_palette(ctx, advacolor[63], 0, 128, 96, 1, "advacolor[63]");
    assert_palette(ctx, advacolor2[33], 192, 224, 224, 1, "advacolor2[33]");
    assert_palette(ctx, gamecolor[15], 224, 224, 0, 1, "gamecolor[15]");
    assert_palette(ctx, zone4colora[23], 0, 0, 32, 1, "zone4colora[23]");
    ...
    TEST_ASSERT_TRUE(ctx, colortbl[4].PeSrc == zone4colora);
    TEST_ASSERT_EQ_INT(ctx, 16, colortbl[4].offset);
    TEST_ASSERT_EQ_INT(ctx, 48, colortbl[4].cnt);

    TEST_ASSERT_EQ_INT(ctx, 32, col4a0cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 50, col4a1cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 51, col4a2cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 52, col4a3cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 57, col4a4cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 58, col4a5cnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 49, col4a6cnt[0]);
    assert_palette(ctx, col4a0col[1], 160, 64, 0, 1, "col4a0col[1]");
    assert_palette(ctx, col4a3col[4], 32, 64, 128, 1, "col4a3col[4]");
    assert_palette(ctx, col4a6col[3], 96, 128, 224, 1, "col4a6col[3]");
}
...
static void test_clchgctr_low_area_updates_six_cycle_slots(test_context *ctx) {
    reset_col4a_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[4]);
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[5]);
    TEST_ASSERT_EQ_INT(ctx, 8, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 8, clchgtim[5]);
    assert_palette(ctx, colorwk[32], 160, 64, 0, 1, "low cycle 32");
    assert_palette(ctx, colorwk[50], 224, 64, 160, 1, "low cycle 50");
    assert_palette(ctx, colorwk[51], 128, 0, 64, 1, "low cycle 51");
    assert_palette(ctx, colorwk[52], 192, 32, 128, 1, "low cycle 52");
    assert_palette(ctx, colorwk[57], 192, 96, 96, 1, "low cycle 57");
    assert_palette(ctx, colorwk[58], 160, 32, 64, 1, "low cycle 58");
}

static void test_clchgctr_high_area_updates_four_slots_and_palette(
    test_context *ctx) {
    reset_col4a_state();
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[2]);
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[3]);
    TEST_ASSERT_EQ_INT(ctx, 0, clchgcnt[4]);
    TEST_ASSERT_EQ_INT(ctx, 2, clchgtim[0]);
    TEST_ASSERT_EQ_INT(ctx, 128, clchgtim[4]);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 1, "high colorset start");
    assert_palette(ctx, colorwk[49], 160, 192, 224, 1, "high cycle 49");
    assert_palette(ctx, colorwk[50], 128, 160, 224, 1, "high cycle 50");
    assert_palette(ctx, colorwk[51], 96, 128, 224, 1, "high cycle 51");
    assert_palette(ctx, colorwk[52], 224, 224, 224, 1, "high cycle 52");
    ...
    TEST_ASSERT_EQ_INT(ctx, 128, clchgtim[4]);
    assert_palette(ctx, colorwk[16], 0, 0, 0, 0, "high skip colorset");
    assert_palette(ctx, colorwk[49], 160, 192, 224, 1,
                   "high preflagged cycle 49");
}

static void test_clchgctr_stage_and_horizontal_skip_to_high_cycle(
    test_context *ctx) {
    reset_col4a_state();
    stageno.b.l = 1;
    clchgtim[4] = 7;
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 7, clchgtim[4]);
    assert_palette(ctx, colorwk[49], 160, 192, 224, 1, "stage high cycle");
    ...
    TEST_ASSERT_EQ_INT(ctx, 1, clchgcnt[0]);
    TEST_ASSERT_EQ_INT(ctx, 128, clchgtim[4]);
    assert_palette(ctx, colorwk[49], 160, 192, 224, 1, "horizontal cycle");
}
...
static void test_colorset_routes_to_all_palette_work_buffers(test_context *ctx) {
    ...
    assert_palette(ctx, colorwk[63], 128, 96, 96, 1, "colorset2 work 0 end");

    colorset3(0);
    assert_palette(ctx, colorwk3[0], 0, 0, 0, 1, "colorset3 work 2");
...
TEST_MAIN_BEGIN;
test_col4a_tables_capture_literal_palette_data(&ctx);
test_clchg_sub0_waits_updates_and_wraps(&ctx);
test_clchgctr_low_area_updates_six_cycle_slots(&ctx);
test_clchgctr_high_area_updates_four_slots_and_palette(&ctx);
test_clchgctr_stage_and_horizontal_skip_to_high_cycle(&ctx);
